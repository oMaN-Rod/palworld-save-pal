...
use ps_core::domain::{containers, pal, player};
use ps_core::dto::container::{ItemContainerDto, ItemContainerSlotDto};
use ps_core::error::CoreError;
use ps_core::gamedata::GameData;
use ps_core::progress::null_progress;
use ps_core::props;
use ps_core::session::{PlayerFileData, SaveKind, SaveSession};

use ps_app::plugin_registry::BUNDLED;
use ps_plugin::manifest::Manifest;
use ps_plugin::runtime::{run_command, RunOutcome, RunRequest, RunServices};
use ps_plugin::sandbox::{Cancel, Limits};
use ps_plugin::status::RunStatus;
...
fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("ps-app has a parent directory")
        .to_path_buf()
}
...
fn count_raw_keys_in_properties(
    properties: &ps_core::ue::Properties,
...
fn count_raw_keys_in_property(
    property: &ps_core::ue::Property,
    wanted: &std::collections::BTreeSet<&str>,
    count: &mut usize,
) {
    use ps_core::ue::{Property, ValueVec};
    match property {
        Property::Struct(sv) => {
...
/// `RawData` fields decode into a typed [`ps_core::ue::games::palworld::PalStruct`]
...
fn game_struct_properties(sv: &ps_core::ue::StructValue) -> Option<&ps_core::ue::Properties> {
    use ps_core::ue::games::palworld::{PalDynamicItemType, PalMapConcreteModelVariant, PalStruct};
    use ps_core::ue::StructValue;
    match sv {
...
fn seed_base_with_no_visible_members(h: &mut Harness) -> Uuid {
    ...
    let ps_core::ue::Property::Struct(ps_core::ue::StructValue::Game(
        ps_core::ue::PalStruct::BaseCamp(camp),
    )) = raw_data
    else {
    ...
    ps_core::domain::world::base_camp_map_mut(&mut h.session.level)
...
fn world_now_tick(session: &SaveSession) -> i64 {
    let world_props = ps_core::domain::world::world_props(&session.level)
...
/// entry -- the same field `ps-core`'s own `transfer::sync_timestamps` writes.
...
/// read or was told about (see `ps-core/tests/schema_priming.rs`), and
/// `LastOnlineRealTime` is absent from `ps-core`'s own curated
/// `save_parameter_schemas()` list precisely because nothing in `ps-core`
...
fn seed_last_online_real_time(session: &mut SaveSession, uid: Uuid, value: i64) {
    props::ensure_schema(
        &mut session.level,
        format!("{}.LastOnlineRealTime", ps_core::domain::pal::LEVEL_SAVE_PARAMETER_PREFIX),
        ps_core::ue::PropertyTagPartial {
            id: None,
            data: ps_core::ue::PropertyTagDataPartial::Other(ps_core::ue::PropertyType::Int64Property),
        },
    );
    let entries =
        ps_core::domain::world::character_map_mut(&mut session.level).expect("character map");
    let entry = entries
        .iter_mut()
        .find(|entry| {
            ps_core::domain::world::entry_is_player(entry)
                && ps_core::domain::world::entry_player_uid(entry) == Some(uid)
        })
        .expect("the player has a character entry");
    let save_parameter = ps_core::domain::world::entry_save_parameter_mut(entry)
...
fn character_entry_exists(session: &SaveSession, instance_id: Uuid) -> bool {
    session
        ...
        .iter()
        .any(|entry| ps_core::domain::world::entry_instance_id(entry) == Some(instance_id))
}
...
fn seed_duplicate_player(session: &mut SaveSession) -> (Uuid, Uuid, Uuid) {
    ...
    let kept_instance = session
        ...
        .and_then(ps_core::domain::world::entry_instance_id)
    ...
    let mut cloned_entry = session
        ...
        .expect("character map must resolve")
        .iter()
        .find(|entry| ps_core::domain::world::entry_instance_id(entry) == Some(kept_instance))
        .cloned()
    ...
    if let Some(save_parameter) = ps_core::domain::world::entry_save_parameter_mut(&mut cloned_entry) {
        save_parameter.insert("LastOnlineRealTime", props::int64_property(world_tick - 1_000_000));
    }

    ps_core::domain::world::character_map_mut(&mut session.level)
...
fn read_last_online_real_time(session: &SaveSession, uid: Uuid) -> Option<i64> {
    ...
    let save_parameter = ps_core::domain::world::entry_save_parameter(entry)?;
...
const BOGUS_ITEM_ID: &str = "PS_Test_Definitely_Bogus_Item";
...
fn seed_bogus_item(h: &mut Harness) -> (Uuid, i32) {
    ...
    .find(|slot| {
        slot.dynamic_item.is_none()
            && matches!(slot.local_id, None | Some(ps_core::props::EMPTY_UUID))
    })
...
fn world_pal_fields(session: &SaveSession) -> Vec<WorldPalFields> {
    ...
    entries
        .iter()
        .filter(|entry| !ps_core::domain::world::entry_is_player(entry))
        .filter_map(|entry| {
            let save_parameter = ps_core::domain::world::entry_save_parameter(entry)?;
            let character_id =
...
fn seed_imported_pal(session: &mut SaveSession) {
    ...
    let entry = entries
        ...
        .find(|entry| {
            !ps_core::domain::world::entry_is_player(entry)
                && ps_core::domain::world::entry_save_parameter(entry).is_some_and(|save_parameter| {
...
fn dps_slot_count(dps_bytes: &[u8]) -> usize {
    let save = ps_core::savio::read_sav_bytes(dps_bytes).expect("the dps bytes parse");
...
fn dps_slot_character_id(dps_bytes: &[u8], slot_index: i32) -> Option<String> {
    ...
    let ps_core::ue::StructValue::Struct(slot_props) = array.get(slot_index as usize)? else {
        return None;
    };
    let save_parameter = slot_props
        .0
        .get(&ps_core::ue::PropertyKey::from("SaveParameter"))
...
fn seed_dps_imported_pal(h: &mut Harness, uid: Uuid) -> i32 {
    ...
    props::ensure_schema(
        ...
        ps_core::ue::PropertyTagPartial {
            id: None,
            data: ps_core::ue::PropertyTagDataPartial::Other(ps_core::ue::PropertyType::BoolProperty),
        },
    );
    ...
    let ps_core::ue::StructValue::Struct(slot_props) = &mut array[slot_index as usize] else {
        panic!("dps slot is not a struct");
    };
    let save_parameter = slot_props
        .0
        .get_mut(&ps_core::ue::PropertyKey::from("SaveParameter"))
...
const BOGUS_PASSIVE_ID: &str = "PS_NOT_A_REAL_PASSIVE";
...
fn count_passive_entries(session: &SaveSession) -> usize {
    ...
    entries
        .iter()
        .filter(|entry| !ps_core::domain::world::entry_is_player(entry))
        .filter_map(ps_core::domain::world::entry_save_parameter)
...
fn passives_of(session: &SaveSession, target: Uuid) -> Vec<String> {
    ...
    entries
        ...
        .find(|entry| {
            !ps_core::domain::world::entry_is_player(entry)
                && ps_core::domain::world::entry_instance_id(entry) == Some(target)
        })
        .and_then(ps_core::domain::world::entry_save_parameter)
...
fn seed_passive_skill(session: &mut SaveSession, skill_id: &str) -> Uuid {
    ...
    let instance_id =
        ps_core::domain::world::entry_instance_id(entry).expect("pal entry has an instance id");
    let save_parameter = ps_core::domain::world::entry_save_parameter_mut(entry)
        .expect("the pal entry has a SaveParameter bag");
...
fn dps_slot_passive_skills(dps_bytes: &[u8], slot_index: i32) -> Vec<String> {
    ...
    let array = props::get(&save.root.properties, &["SaveParameterArray"])
        .and_then(props::struct_values)
        .expect("SaveParameterArray present");
    let ps_core::ue::StructValue::Struct(slot_props) =
...
fn count_map_objects(session: &SaveSession) -> usize {
    ps_core::domain::map_object::map_object_ids(session).expect("map object ids").len()
}

fn count_map_objects_where(
    session: &SaveSession,
    predicate: impl Fn(&ps_core::domain::map_object::MapObjectView) -> bool,
) -> usize {
    ps_core::domain::map_object::map_object_views(session)
...
fn count_work_entries(session: &SaveSession) -> usize {
    ps_core::domain::world::work_values(&session.level)
        .expect("work values")
        .map(|values| values.len())
        .unwrap_or(0)
}
...
/// `ps_core::domain::map_object`, which is what the command under test calls.
fn work_owner_uuid(value: &ps_core::ue::StructValue) -> Option<Uuid> {
    let ps_core::ue::StructValue::Struct(properties) = value else { return None };
    match properties.0.get(&ps_core::ue::PropertyKey::from("RawData"))? {
        ps_core::ue::Property::Struct(ps_core::ue::StructValue::Game(
            ps_core::ue::PalStruct::Work(work),
...
fn count_orphaned_works(session: &SaveSession) -> usize {
    let surviving: std::collections::HashSet<Uuid> =
        ps_core::domain::map_object::map_object_ids(session).expect("map object ids").into_iter().collect();
    ps_core::domain::world::work_values(&session.level)
        .expect("work values")
        .map(|values| {
            values
...
fn orphan_one_base(session: &mut SaveSession) -> (Uuid, usize) {
    ...
    ps_core::domain::world::base_camp_map_mut(&mut session.level)
        .expect("base camp map")
        .expect("base camp map")
...
/// `ps_core::props` reaches it directly, since it lives in the outer
/// property bag rather than inside the typed `RawData` model.
fn seed_map_object_id(session: &mut SaveSession, name: &str) {
    let objects = ps_core::domain::world::map_object_values_mut(&mut session.level)
    ...
    let ps_core::ue::StructValue::Struct(properties) = object else {
...
fn base_worker_container_map(session: &SaveSession) -> BTreeMap<Uuid, Uuid> {
    ...
    for entry in entries {
        ...
        if let Some((_, container_id)) = ps_core::domain::guild::base_guild_and_container(entry) {
...
fn map_object_builder(session: &SaveSession, id: Uuid) -> Option<Uuid> {
    ps_core::domain::map_object::read_map_object(session, id).and_then(|v| v.build_player_uid)
}
...
fn seed_map_object_built_by_missing_player(session: &mut SaveSession) -> (Uuid, Uuid) {
    let target = ps_core::domain::map_object::map_object_views(session)
        .expect("map object views")
        .into_iter()
    ...
    let wrote =
        ps_core::domain::map_object::set_map_object_builder(session, target.instance_id, Some(ghost_uid))
            .expect("write");
...
fn dynamic_item_ids(session: &SaveSession) -> std::collections::HashSet<Uuid> {
    ...
    ps_core::domain::world::dynamic_item_values(&session.level)
...
fn referenced_dynamic_item_ids(session: &SaveSession) -> std::collections::HashSet<Uuid> {
    use ps_core::ue::games::palworld::{PalItemId, PalMapConcreteModelVariant};
    use ps_core::ue::{PalStruct, Property, PropertyKey, StructValue};
    ...
    fn concrete_variant(
        object: &StructValue,
    ) -> Option<&PalMapConcreteModelVariant<ps_core::ue::Arch>> {
    ...
    if let Ok(Some(objects)) = ps_core::domain::world::map_object_values(&session.level) {
...
fn setup_for(command_id: &str, h: &mut Harness) -> serde_json::Value {
    match command_id {
        ...
        "remove_invalid_pals_from_save" => {
            seed_pal_with_character_id(h, "PS_NOT_A_REAL_PAL");
            serde_json::json!({})
        }
        "delete_inactive_bases" => {
        ...
        "delete_invalid_structure_map_objects" => {
            seed_map_object_id(&mut h.session, "PS_NOT_A_REAL_STRUCTURE");
            serde_json::json!({})
        }
...
fn remove_invalid_pals_deletes_a_seeded_unknown_species() {
    let mut h = Harness::new();
    let before = count_pals_where(&h.session, |_| true);
    seed_pal_with_character_id(&mut h, "PS_NOT_A_REAL_PAL");
...
fn remove_invalid_passives_strips_a_seeded_unknown_and_keeps_the_rest() {
    let mut h = Harness::new();
    let target = seed_passive_skill(&mut h.session, "PS_NOT_A_REAL_PASSIVE");
    let before = count_passive_entries(&h.session);
    assert!(before > 1, "the fixture must carry real passives too");
    assert!(
        passives_of(&h.session, target).iter().any(|p| p == "PS_NOT_A_REAL_PASSIVE"),
...
fn both_bundled_plugins_are_registered_with_distinct_ids() {
    let ids: Vec<&str> = ps_app::plugin_registry::BUNDLED.iter().map(|p| p.id).collect();
