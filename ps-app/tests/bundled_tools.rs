...
use ps_core::domain::guild::{base_camp_location, guild_chest_id};
use ps_core::domain::map_object;
...
fn find_occupied_container(session: &mut SaveSession, game_data: &GameData) -> (Uuid, i32, i32) {
    let ids: Vec<Uuid> = session
        ...
        .iter()
        .filter_map(|entry| {
            ps_core::props::get(ps_core::props::struct_props(&entry.key)?, &["ID"])
...
/// The lock count is read back through `ps_core::domain::map_object`
...
fn dps_occupied_levels(h: &Harness, uid: Uuid) -> BTreeMap<usize, u8> {
    ...
    let array = ps_core::props::get(&save.root.properties, &["SaveParameterArray"])
    ...
    array
        ...
        .filter_map(|(index, slot)| {
            let ps_core::ue::StructValue::Struct(slot_props) = slot else { return None };
            ...
            let level = ps_core::props::get(save_parameter, &["Level"]).and_then(ps_core::props::as_byte)?;
            Some((index, level))
        })
        .collect()
}
