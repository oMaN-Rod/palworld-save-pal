...
use ps_core::domain::player;
use ps_core::dto::ordered_map::OrderedMap;
...
fn the_bundled_view_wires_an_entity_select_a_button_and_a_selectable_table() {
    ...
    let widgets: Vec<&ps_plugin::manifest::UiWidget> =
...
fn repair_structures_raises_every_damaged_map_object_to_full_hp() {
    ...
    // ps-core/src/domain/map_object.rs:13. The Lua handle's fields can still
    // read nil, which is what the command's own guard is for.
    let damaged_before = ps_core::domain::map_object::map_object_views(&h.session)
    ...
    let still_damaged = ps_core::domain::map_object::map_object_views(&h.session)
        .expect("map objects read")
        .iter()
        .filter(|v| v.hp < v.max_hp)
        .count();
...
fn repair_structures_under_a_dry_run_changes_nothing_but_reports_the_same_count() {
    let mut h = Harness::new();
    let before = ps_core::domain::map_object::map_object_views(&h.session)
        .expect("map objects read")
        .iter()
    ...
    let after = ps_core::domain::map_object::map_object_views(&h.session)
        .expect("map objects read")
        .iter()
        .map(|v| (v.instance_id, v.hp))
        .collect::<std::collections::BTreeMap<_, _>>();
...
/// seeding goes through `ps_core::domain::player` directly -- there is no
...
fn teach_a_pal_an_unlearnable_skill(h: &mut Harness) -> (Uuid, String) {
    ...
    let catalogued: std::collections::BTreeSet<String> = game_data
        ...
        .map(str::to_string)
        .collect();
    let pal_keys = ps_core::domain::pal::known_pal_keys(game_data).clone();
    ...
    for entry in entries.iter_mut() {
        ...
        let key = ps_core::dto::pal::format_character_key(&character_id, &pal_keys);
...
fn equipped_skills(session: &SaveSession, pal_id: Uuid) -> Vec<String> {
    ps_core::domain::world::character_map(&session.level)
        ...
        .iter()
        .find(|entry| ps_core::domain::world::entry_instance_id(entry) == Some(pal_id))
        .and_then(ps_core::domain::world::entry_save_parameter)
        .and_then(|params| params.0.get(&ps_core::ue::PropertyKey::from("EquipWaza")))
        .and_then(ps_core::props::enum_values)
        .cloned()
        .unwrap_or_default()
}
...
fn seed_sick_pals(session: &mut SaveSession, count: usize) -> usize {
    let mut seeded = 0;
    for entry in ps_core::domain::world::character_map_mut(&mut session.level)
        .expect("the character map resolves")
        .iter_mut()
    {
        if seeded == count {
        ...
        if ps_core::domain::world::entry_is_player(entry) {
            continue;
        }
        let Some(params) = ps_core::domain::world::entry_save_parameter_mut(entry) else {
            continue;
        };
        params.insert("WorkerSick", ps_core::props::bool_property(true));
        params.insert("SanityValue", ps_core::props::float_property(3.0));
        seeded += 1;
    }
    seeded
}
...
fn orphan_a_contained_pal(h: &mut Harness) -> (Uuid, Uuid) {
    ...
    let entries = ps_core::domain::world::character_map_mut(&mut h.session.level)
        .expect("the character map resolves");
    let entry = entries
        .iter_mut()
        .find(|entry| ps_core::domain::world::entry_instance_id(entry) == Some(pal_id))
        .expect("the pal the container names must be in the character map");
    let params = ps_core::domain::world::entry_save_parameter_mut(entry).expect("save parameter");
    let previous = params
        .into_iter()
        .find(|(key, _)| key.1.as_str() == "OwnerPlayerUId")
        .and_then(|(_, value)| ps_core::props::as_uuid(value))
        .expect("a pal in a player container has an owner to begin with");
    params
        .0
        .shift_remove(&ps_core::ue::PropertyKey::from("OwnerPlayerUId"));
    (pal_id, previous)
}
...
fn break_one_item_link(session: &mut SaveSession) -> (Uuid, i32) {
        ...
        .find_map(|entry| {
            ...
            slots.iter().find_map(|slot| {
                ...
                let Some(Property::Struct(StructValue::Game(
                    ps_core::ue::PalStruct::ItemContainerSlots(raw),
    ...
    values.retain(|value| {
        ...
        !matches!(
            ...
            Some(Property::Struct(StructValue::Game(ps_core::ue::PalStruct::DynamicItem(item))))
...
fn container_holds_slot(session: &mut SaveSession, game_data: &GameData, container_id: Uuid, slot_index: i32) -> bool {
    ps_core::domain::containers::read_item_container(
        &session.level,
        &mut session.caches,
        game_data,
        container_id,
...
fn misfile_a_pals_guild(h: &mut Harness) -> (Uuid, Uuid) {
    ...
    for entry in entries.iter_mut() {
        ...
        let Some(pal_id) = ps_core::domain::world::entry_instance_id(entry) else { continue };
        let Some(data) = ps_core::domain::world::entry_character_data_mut(entry) else { continue };
        let belongs_to = ps_core::props::guid_to_uuid(&data.group_id);
        if belongs_to.is_nil() || belongs_to == wrong {
            continue;
        }
        data.group_id = ps_core::props::uuid_to_guid(wrong);
...
fn guild_of(session: &SaveSession, pal_id: Uuid) -> Option<Uuid> {
    ps_core::domain::world::character_map(&session.level)
        ...
        .iter()
        .find(|entry| ps_core::domain::world::entry_instance_id(entry) == Some(pal_id))
        .and_then(ps_core::domain::world::entry_character_data)
