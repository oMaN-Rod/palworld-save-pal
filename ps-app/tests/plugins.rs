...
use ps_app::dispatcher::HandlerCtx;
use ps_app::handlers::plugins::*;
use ps_app::test_support::TestContext;
use ps_db::plugins::NewPlugin;
...
fn minimal_save() -> ps_core::session::SaveSession {
    use ps_core::ue::*;
...
async fn list_plugins_on_a_fresh_database_returns_the_bundled_set() {
    ...
    assert_eq!(list.len(), ps_app::plugin_registry::BUNDLED.len());
}
...
async fn clone_plugin_creates_a_user_copy_with_its_own_id_and_name() {
    ...
    let cloned = ps_db::plugins::get(&*test.app.driver, "source.plugin-copy")
        .await
        .unwrap()
        .expect("cloned plugin exists");
    let manifest = ps_plugin::manifest::Manifest::parse(&cloned.manifest).unwrap();
...
async fn create_plugin_writes_a_runnable_scaffold() {
    ...
    let manifest = ps_plugin::manifest::Manifest::parse(&row.manifest)
...
fn seed_player(session: &mut ps_core::session::SaveSession, nickname: &str) -> uuid::Uuid {
    let uid = uuid::Uuid::new_v4();
    session.player_summaries.insert(
        uid,
        ps_core::dto::summary::PlayerSummary {
            uid,
