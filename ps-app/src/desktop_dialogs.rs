...
pub fn validate_selected_file(
    ...
    if selected.starts_with(app_root) {
        return Err(
            "Selected path is inside the PalStudio application directory. Please move your save files outside of the application directory."
                .to_string(),
        );
    }
...
mod tests {
    ...
    fn selection_inside_app_root_is_rejected() {
        ...
        assert_eq!(
            validate_selected_file("steam", selected, app_root),
            Err("Selected path is inside the PalStudio application directory. Please move your save files outside of the application directory.".to_string())
        );
    }
