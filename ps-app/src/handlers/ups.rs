//! UPS pal handlers, backed by the ps-db UPS layer.

use crate::dispatcher::HandlerCtx;
use crate::handler_error::HandlerError;
use crate::messages::MessageType;
use ps_core::domain::pal;
use ps_core::domain::player::build_player_dto;
...
pub struct AddUpsPalData {
    pub pal_dto: ps_core::dto::pal::PalDto,
...
fn pal_wire_object(
    ...
    serde_json::json!({
        ...
        "character_key": ps_core::dto::pal::format_character_key(&record.character_id, known_keys),
...
pub async fn handle_get_ups_all_filtered_ids(
    ...
    match ps_db::ups::get_all_filtered_ids(&*ctx.app.driver, &filter).await {
...
pub(crate) fn new_ups_pal_from_dto(
    data: AddUpsPalData,
) -> Result<ps_db::ups::NewUpsPal, HandlerError> {
    let pal_data = serde_json::to_value(&data.pal_dto)?;
    Ok(ps_db::ups::NewUpsPal {
...
pub async fn handle_add_ups_pal(
    ...
    match ps_db::ups::add_pal(&*ctx.app.driver, new_pal, &pals_data).await {
...
pub async fn handle_update_ups_pal(
    ...
    match ps_db::ups::update_pal(&*ctx.app.driver, data.pal_id, &data.updates).await {
...
pub async fn handle_delete_ups_pals(
    ...
    let pals_data = pals_game_data(ctx);
    match ps_db::ups::delete_pals(&*ctx.app.driver, &data.pal_ids, &pals_data).await {
...
pub async fn handle_clone_ups_pal(
    ...
    let pals_data = pals_game_data(ctx);
    match ps_db::ups::clone_pal(&*ctx.app.driver, data.pal_id, &pals_data).await {
...
pub async fn handle_get_ups_stats(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    let pals_data = pals_game_data(ctx);
    match ps_db::ups::get_stats(&*ctx.app.driver, &pals_data).await {
...
pub async fn handle_nuke_ups_pals(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    let pals_data = pals_game_data(ctx);
    let (_, total_count) = match ps_db::ups::get_pals(
        &*ctx.app.driver,
        &UpsFilter::default(),
        "created_at",
        "desc",
    ...
    match ps_db::ups::nuke_all_pals(&*ctx.app.driver, &pals_data).await {
        Ok(deleted_count) => ctx.emitter.emit(
            MessageType::NukeUpsPals,
            &serde_json::json!({
                "success": true,
...
pub async fn handle_get_ups_collections(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    match ps_db::ups::get_collections(&*ctx.app.driver).await {
...
pub async fn handle_create_ups_collection(
    ...
    match ps_db::ups::create_collection(
        &*ctx.app.driver,
        &data.name,
...
pub async fn handle_update_ups_collection(
    ...
    match ps_db::ups::update_collection(&*ctx.app.driver, data.collection_id, &data.updates).await
    {
...
pub async fn handle_delete_ups_collection(
    ...
    match ps_db::ups::delete_collection(&*ctx.app.driver, data.collection_id).await {
...
pub async fn handle_get_ups_tags(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    match ps_db::ups::get_available_tags(&*ctx.app.driver).await {
...
pub async fn handle_create_ups_tag(
    ...
    match ps_db::ups::create_or_update_tag(
        &*ctx.app.driver,
        &data.name,
        data.description.as_deref(),
        data.color.as_deref(),
...
pub async fn handle_update_ups_tag(
    ...
    match ps_db::ups::update_tag(&*ctx.app.driver, data.tag_id, &data.updates).await {
...
pub async fn handle_delete_ups_tag(
    ...
    match ps_db::ups::delete_tag(&*ctx.app.driver, data.tag_id).await {
        Ok(success) => ctx.emitter.emit(
...
pub async fn handle_export_ups_pal(
    ...
    let Some(ups_pal) = ps_db::ups::get_pal_by_id(&*ctx.app.driver, data.pal_id).await? else {
    ...
    if exported {
        ps_db::ups::export_pal_to_save(
            &*ctx.app.driver,
            data.pal_id,
            &data.destination_type,
            &ps_db::ups::ExportDestinationInfo {
