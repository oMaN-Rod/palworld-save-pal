...
/// Public so `handlers::tools`'s `load_source_save` and ps-server's
...
pub(crate) async fn select_gamepass_directory(
    ...
    let saves = ps_core::gamepass::scan::scan_saves(&containers_dir)?;
    if saves.is_empty() {
        ctx.emitter.emit(
            MessageType::Error,
...
async fn pick_save_file_via_dialog(
    ...
    let saved_dir = ps_db::settings::saved_save_dir(&*ctx.app.driver).await?;
...
pub async fn handle_select_save(
    ...
    if ctx.app.config.desktop_mode && ctx.is_loopback {
        ...
        if let Some(parent_dir) = selected.parent() {
            ...
            // user just picked.
            ps_db::settings::update_save_dir(&*ctx.app.driver, &parent_dir.to_string_lossy()).await?;
        }
    ...
    let mut player_count: usize = 0;
    {
        ...
        if let Some(world_option) = &session.world_option {
            zip_writer
                ...
                .map_err(|zip_error| HandlerError::Other(zip_error.to_string()))?;
            zip_writer
                .write_all(&ps_core::savio::write_sav_bytes(world_option)?)
                .map_err(CoreError::Io)?;
        }
    ...
    if session.world_option_dirty {
        if let Some(world_option) = &session.world_option {
            ...
            std::fs::write(
                ...
                ps_core::savio::write_sav_bytes(world_option)?,
            )
            .map_err(CoreError::Io)?;
        }
    }
...
pub(crate) fn write_transfer_target_save(
    session: &SaveSession,
    save_info: &ps_core::session::TransferSaveInfo,
) -> Result<(), HandlerError> {
    let level_sav_bytes = session.level_sav_bytes()?;
...
fn steam_backup_base() -> std::path::PathBuf {
    ps_core::paths::app_root().join("backups").join("steam")
}
...
async fn save_modded_steam_save(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    let save_dir = ps_db::settings::get_settings(&*ctx.app.driver).await?.save_dir;
    let progress = ctx.emitter.progress_sink();
...
async fn write_modded_gamepass_containers(
    ...
    use ps_core::gamepass::format::ContainerIndex;
    use ps_core::gamepass::{store, PlayerSavBytes};
    ...
    let container_dir = PathBuf::from(ps_db::settings::get_settings(&*ctx.app.driver).await?.save_dir);
    let progress = ctx.emitter.progress_sink();
    ...
    let modified_world_option = match (&session.world_option, session.world_option_dirty) {
        (Some(save), true) => Some(ps_core::savio::write_sav_bytes(save)?),
        _ => None,
    };
...
pub async fn handle_save_edited_sav(
    ...
    let sav_bytes = match ps_core::convert::json_to_sav_bytes(data.json.as_bytes()) {
        Ok(bytes) => bytes,
        Err(error) => {
            ctx.emitter.emit(
                MessageType::SaveEditedSav,
...
pub async fn handle_convert_sav_file(
    ...
    if data.target_type == "json" {
        let json_text = ps_core::convert::sav_to_json_string(&data.file_data)?;
        ctx.emitter.emit(MessageType::ConvertSavFile, &json_text);
    } else {
        let sav_bytes = ps_core::convert::json_to_sav_bytes(&data.file_data)?;
...
fn unlock_map_on_disk(data: &UnlockMapData) -> Result<(), String> {
    ...
    let outcome =
        ps_core::localdata::unlock_world_map(&local_data).map_err(|error| error.to_string())?;
