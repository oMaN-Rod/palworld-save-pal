...
use ps_plugin::manifest::{Capability, CommandDef, Manifest};
...
pub struct PluginSummary {
    ...
    pub ui: Vec<ps_plugin::manifest::UiSection>,
...
fn summarize(row: &ps_db::plugins::PluginRow) -> PluginSummary {
...
pub async fn handle_list_plugins(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    let rows = ps_db::plugins::get_all(&*ctx.app.driver).await?;
...
fn player_options(session: &ps_core::session::SaveSession) -> Vec<EntityOption> {
    session
...
fn guild_options(session: &ps_core::session::SaveSession) -> Vec<EntityOption> {
    session
        .guild_summary_order
        .iter()
...
fn base_options(session: &ps_core::session::SaveSession) -> Vec<EntityOption> {
    let entries = session.base_camp_map().unwrap_or(&[]);
    entries
        .iter()
        .filter_map(|entry| {
            let id = ps_core::props::as_uuid(&entry.key)?;
            let guild = ps_core::domain::guild::base_guild_and_container(entry)
...
fn pal_options(
    session: &ps_core::session::SaveSession,
    game_data: &ps_core::gamedata::GameData,
) -> Vec<EntityOption> {
    let Ok(summaries) = ps_core::domain::pal::pal_summaries(session, game_data) else {
        return Vec::new();
    };
    summaries
        .iter()
...
pub async fn handle_get_plugin(
    ...
    let Some(row) = ps_db::plugins::get(&*ctx.app.driver, &data.id).await? else {
        ctx.emitter.emit(
            MessageType::Error,
...
pub async fn handle_install_plugin(
    ...
    if let Some(existing) = ps_db::plugins::get(&*ctx.app.driver, &manifest.id).await? {
        if existing.bundled {
            return install_error(
                ctx,
    ...
    let row = ps_db::plugins::upsert(
        &*ctx.app.driver,
        &ps_db::plugins::NewPlugin {
            id: &manifest.id,
...
fn plugin_archive_bytes(row: &ps_db::plugins::PluginRow) -> Result<Vec<u8>, HandlerError> {
...
pub async fn handle_uninstall_plugin(
    ...
    ps_db::plugins::remove(&*ctx.app.driver, &data.id).await?;
    ctx.emitter.emit(
...
pub async fn handle_clone_plugin(
    ...
    if ps_db::plugins::get(&*ctx.app.driver, &data.target_id).await?.is_some() {
        return clone_refused(ctx, format!("plugin {:?} already exists", data.target_id));
    }
    let Some(source) = ps_db::plugins::get(&*ctx.app.driver, &data.source_id).await? else {
    ...
    let cloned = ps_db::plugins::upsert(
        &*ctx.app.driver,
        &ps_db::plugins::NewPlugin {
            id: &manifest.id,
    ...
    .await?;
    if !source.enabled {
        ps_db::plugins::set_enabled(&*ctx.app.driver, &manifest.id, false).await?;
    }
    let row = ps_db::plugins::get(&*ctx.app.driver, &manifest.id)
        .await?
        .unwrap_or(cloned);
...
pub async fn handle_set_plugin_enabled(
    ...
    let found = ps_db::plugins::set_enabled(&*ctx.app.driver, &data.id, data.enabled).await?;
    if !found {
        return install_error(ctx, format!("plugin {} not found", data.id));
    }
    let rows = ps_db::plugins::get_all(&*ctx.app.driver).await?;
...
fn log_level_wire(level: ps_plugin::context::LogLevel) -> &'static str {
    use ps_plugin::context::LogLevel;
    match level {
        LogLevel::Info => "info",
        LogLevel::Warn => "warn",
...
async fn run_plugin(
    ...
    let Some(row) = ps_db::plugins::get(&*ctx.app.driver, plugin_id).await? else {
    ...
    let storage = ps_db::plugins::storage_get_all(&*ctx.app.driver, plugin_id).await?;
    ...
    if outcome.status.is_ok() && !outcome.storage_writes.is_empty() {
        ps_db::plugins::storage_put_many(&*ctx.app.driver, plugin_id, &outcome.storage_writes)
            .await?;
    }
...
pub async fn handle_check_plugin_syntax(
    ...
    let error = ps_plugin::syntax::check(&data.source);
    ctx.emitter
...
pub async fn handle_get_api_definition(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    ctx.emitter
        .emit(MessageType::GetApiDefinition, &ps_plugin::api_definition());
    Ok(())
}
...
pub async fn handle_create_plugin(
    ...
    if ps_db::plugins::get(&*ctx.app.driver, &data.id).await?.is_some() {
...
pub async fn handle_delete_plugin_source(
    ...
    ps_db::plugins::set_sources(&*ctx.app.driver, &data.id, &serde_json::to_string(&sources)?)
        .await?;
