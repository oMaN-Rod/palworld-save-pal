...
pub async fn handle_get_presets(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    ...
    ps_db::presets::populate_from_json(&*ctx.app.driver, &seed).await?;
    let presets = ps_db::presets::get_all(&*ctx.app.driver).await?;
...
pub async fn handle_add_preset(
    ...
    let preset_id = ps_db::presets::add(&*ctx.app.driver, data).await?;
    ctx.emitter.emit(
...
pub async fn handle_update_preset(
    ...
    let success = ps_db::presets::update_name(&*ctx.app.driver, &data.id, &data.name).await?;
    if success {
        ctx.emitter.emit(
...
pub async fn handle_delete_presets(
    ...
    for preset_id in &data {
        if !ps_db::presets::delete(&*ctx.app.driver, preset_id).await? {
            success = false;
        }
    }
    if success {
...
pub async fn handle_nuke_presets(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    ps_db::presets::nuke(&*ctx.app.driver).await?;
    ctx.emitter
...
async fn import_preset_value(
    ...
    for mut preset in items {
        ...
        ps_db::presets::add(db, preset).await?;
        imported += 1;
    }
    Ok(imported)
}
...
mod tests {
    ...
    async fn import_preset_value_adds_single_object_and_strips_id() {
        ...
        let presets = ps_db::presets::get_all(&*test.app.driver).await.unwrap();
        assert_eq!(presets.len(), 1);
