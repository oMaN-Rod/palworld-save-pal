...
pub fn export_file(session: &SaveSession, name: &str) -> Result<Vec<u8>, HandlerError> {
    ...
    match name {
        ...
        _ => {
            ...
            Ok(ps_core::savio::write_gvas_bytes(save)?)
        }
    }
}
...
mod tests {
    ...
    fn world1_level_gvas_bytes() -> Vec<u8> {
        ...
        .expect("read world1");
        let save = ps_core::savio::read_sav_bytes(&sav).expect("parse");
        ps_core::savio::write_gvas_bytes(&save).expect("gvas")
    }
    ...
    async fn staged_bytes_load_and_export_round_trip_without_base64() {
        ...
        let mut arc = std::sync::Arc::new(tokio::sync::Mutex::new(ps_core::session::Session::new()));
    ...
    async fn export_manifest_lists_level_meta_when_one_is_loaded() {
        ...
        let meta = ps_core::savio::write_gvas_bytes(
            &ps_core::savio::read_sav_bytes(&meta_sav).expect("parse meta"),
        )
        .expect("meta gvas");
