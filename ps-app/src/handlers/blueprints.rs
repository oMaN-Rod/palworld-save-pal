...
pub async fn handle_store_blueprint(
    ...
    let row = {
        ...
        ps_db::blueprints::NewBlueprint {
            ...
            payload: ps_core::domain::blueprint::gvas::to_psbp_bytes(blueprint)?,
            preview: None,
        }
    };
    let id = ps_db::blueprints::insert(&*ctx.app.driver, row).await?;
    ctx.emitter
...
pub async fn handle_list_blueprints(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    let blueprints = ps_db::blueprints::list(&*ctx.app.driver).await?;
    ctx.emitter.emit(
...
pub async fn handle_validate_blueprint_placement(
    ...
    use ps_core::domain::blueprint::validate;
...
pub async fn handle_request_blueprint_geometry(
    ...
    let origin = ps_core::domain::blueprint::capture::source_origin(blueprint)
...
pub async fn handle_delete_blueprint(
    ...
    ps_db::blueprints::delete(&*ctx.app.driver, &data.id).await?;
    ctx.emitter
