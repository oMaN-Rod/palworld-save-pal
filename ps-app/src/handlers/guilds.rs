...
pub struct UpdateLabResearchData {
    pub guild_id: uuid::Uuid,
    pub research_updates: Vec<ps_core::dto::guild::GuildLabResearchInfo>,
}
...
pub async fn handle_request_guild_details(
    ...
    match ps_core::domain::guild::get_guild_details(session, &ctx.app.game_data, guild_id)? {
...
pub async fn handle_update_lab_research(
    ...
    match ps_core::domain::guild::update_lab_research(
        session,
        data.guild_id,
        ...
        Err(ps_core::error::CoreError::GuildNotFound(_)) => {
            ctx.emitter.emit(
                MessageType::Error,
...
pub async fn handle_delete_base(
    ...
    ps_core::domain::guild::delete_base(session, &game_data, data.base_id)?;
    ctx.emitter.emit(
...
pub async fn handle_delete_guild(
    ...
    ps_core::domain::guild::delete_guild_and_players(
        session,
        &ctx.app.game_data,
        data.guild_id,
        &progress,
