...
pub async fn handle_sync_app_state(ctx: &mut HandlerCtx<'_>) -> Result<(), HandlerError> {
    let row = ps_db::settings::get_settings(&*ctx.app.driver).await?;
    ctx.emitter
...
mod tests {
    ...
    async fn sync_app_state_without_save_emits_only_settings() {
        ...
        assert_eq!(
            frame["data"]["save_dir"],
            ps_db::settings::default_steam_save_dir()
        );
    ...
    fn fake_loaded_session() -> ps_core::session::SaveSession {
        use ps_core::dto::summary::{GuildSummary, PlayerSummary};
