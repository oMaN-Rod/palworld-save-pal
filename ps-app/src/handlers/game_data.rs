...
pub async fn handle_get_base_structures(
    ...
    let structures = ps_core::domain::guild::base_structures(session, request.base_id);
    ctx.emitter.emit(
        MessageType::GetBaseStructures,
...
mod tests {
    ...
    fn level_with_one_structure() -> ps_core::ue::Save {
        use ps_core::ue::games::palworld::{
        ...
        model_props.insert(
            "RawData",
            Property::Struct(StructValue::Game(ps_core::ue::PalStruct::MapModel(
                Box::new(model),
            ))),
        );
    ...
    async fn set_language(test: &TestContext, language: &str) {
        ps_db::settings::update_settings(
            &*test.app.driver,
            &ps_db::settings::SettingsUpdate {
