...
use ps_core::session::{SaveKind, SaveSession, TransferSaveInfo, TransferTarget};
use uuid::Uuid;
...
async fn resolve_transfer_source_path(ctx: &mut HandlerCtx<'_>) -> Result<Option<String>, String> {
    ...
    if let Some(parent_dir) = selected.parent() {
        ps_db::settings::update_save_dir(&*ctx.app.driver, &parent_dir.to_string_lossy())
            .await
            .map_err(|error| error.to_string())?;
    }
...
pub async fn handle_transfer_player(
    ...
    let options = ps_core::transfer::TransferOptions {
    ...
    let transfer_result = {
        ...
        ps_core::transfer::transfer_player(
            source,
            target,
    ...
    let (succeeded, mut result) = match transfer_result {
        ...
        Err(ps_core::transfer::TransferError::Rejected(message)) => {
            (false, serde_json::json!({"error": message}))
        }
        Err(ps_core::transfer::TransferError::Core(error)) => {
