...
use ps_core::domain::blueprint::BaseBlueprint;
...
mod tests {
    use super::*;
    use ps_core::domain::blueprint::{capture, CaptureOptions};
    use ps_core::props;
    ...
    fn load_session(level_sav: &std::path::Path) -> ps_core::session::SaveSession {
        let level_sav_bytes = std::fs::read(level_sav).expect("read world1 Level.sav");
        ps_core::session::SaveSession::load(
            ps_core::session::SaveKind::Steam {
