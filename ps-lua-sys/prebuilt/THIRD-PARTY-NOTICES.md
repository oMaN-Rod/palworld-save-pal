...
- **Provenance:** built from the vendored sources at
  `ps-lua-sys/vendor/lua-5.4.8/src` by `scripts/build-wasm-lua.ps1` /
  `build-wasm-lua.sh`, using wasi-sdk 33.0's `clang`. Also holds one object
  that is not upstream Lua: `ps-lua-sys/src/shim.c`, this project's own
  code, compiled by the same script into the same archive.
- **Full license text:** `ps-lua-sys/vendor/LUA-LICENSE.txt`
