...
extern "C" {
    ...
    /// name -- `ps-lua-sys` cannot reference `ps-plugin` directly without a
    /// circular rlib dependency. See `ps-lua-sys/src/shim.c`.
    pub fn ps_host_trampoline(state: *mut lua_State) -> c_int;
}
