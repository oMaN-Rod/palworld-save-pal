...
*/
typedef int (*ps_dispatch_fn)(lua_State *L, void *host_fn,
                               const char **message, size_t *len,
                               void **owner);
typedef void (*ps_free_fn)(void *owner);

int ps_host_trampoline(lua_State *L) {
  ps_dispatch_fn dispatch = (ps_dispatch_fn)lua_touserdata(L, lua_upvalueindex(1));
  void *host_fn = lua_touserdata(L, lua_upvalueindex(2));
  ps_free_fn free_message = (ps_free_fn)lua_touserdata(L, lua_upvalueindex(3));
