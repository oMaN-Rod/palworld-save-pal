<script lang="ts">
	...
	function metricPill(player: PlayerRow): string {
		switch (metric) {
			case 'pal_count':
			...
			case 'total_power':
				return compact.format(player.total_power);
			case 'dps_pal_count':
				return player.dps_pal_count == null
					? '—'
					: m.overview_dps_count({ count: player.dps_pal_count.toLocaleString() });
		}
	}
</script>
	...
	<div class="mb-4 flex items-center justify-between gap-2">
		...
		<div class="w-44 shrink-0">
			<Select
				...
				value={metric}
				onChange={(value) => (metric = value as LeaderboardMetric)}
