...
fn decompress_sav_container(sav_bytes: &[u8]) -> Vec<u8> {
    ps_core::ue::compression::decompress_save(&mut std::io::Cursor::new(sav_bytes))
...
fn normalized_member_gvas(compressed_sav: &[u8]) -> Vec<u8> {
    ...
    let mut save = ps_core::savio::read_sav_bytes(compressed_sav).expect("parse sav container");
    ...
    let recompressed = ps_core::savio::write_sav_bytes(&save).expect("re-serialize sav container");
    ps_core::ue::compression::decompress_save(&mut std::io::Cursor::new(recompressed))
...
fn download_world_prefix_parses_the_timestamped_shape() {
    ...
    assert_eq!(
        download_world_prefix("PalStudio_20260710_143012.zip").as_deref(),
        Some("PalStudio")
    );
    // Bad shapes.
...
fn chunk_envelope_shape_is_pinned() {
    use ps_server::signal::framing::chunk_frame;
