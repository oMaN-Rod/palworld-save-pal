...
async fn start_test_server() -> (ps_server::ServerHandle, tempfile::TempDir) {
    let root = repo_root();
...
/// against `ps-server/src/handlers/guilds.rs`), and its response nests the
...
async fn load_of_malformed_base64_content_is_an_error_frame() {
    ...
    let mut socket = connect(server.addr).await;
    send(&mut socket, json!({"type": "load_blueprint",
        "data": {"content": "!!!! not base64 !!!!", "format": "psbp"}})).await;
    let frame = recv(&mut socket).await;
