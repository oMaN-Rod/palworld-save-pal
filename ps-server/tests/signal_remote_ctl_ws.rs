...
async fn install_server_mod_is_refused_and_writes_nothing() {
    ...
    let new_server = ps_db::servers::NewServer {
        ...
        query_port: 27015,
        rest_api_port: 8212,
        data_volume_name: "ps-refusal-test-data".to_string(),
        mods_path: mods_path.to_string_lossy().into_owned(),
        admin_password: "admin".to_string(),
        server_name: "PalStudio Palworld Server".to_string(),
        ..Default::default()
    };
    let record = ps_db::servers::create_server(&*server.handle.app.driver, new_server)
        .await
        .unwrap();
