...
async fn named_extract_canceled_dialog_answers_canceled() {
    let server = common::start_desktop_test_server(Arc::new(
        ps_server::desktop_dialogs::QueuedDialogProvider::new_with_folders(vec![None]),
    ))
    .await;
...
async fn named_extract_writes_steam_tree_to_picked_dir() {
    ...
    let server = common::start_desktop_test_server(Arc::new(
        ps_server::desktop_dialogs::QueuedDialogProvider::new_with_folders(vec![Some(
            steam_out.clone(),
        )]),
    ))
    .await;
...
async fn loaded_gamepass_save_converts_to_steam_dir() {
    ...
    let server = common::start_desktop_test_server(Arc::new(
        ps_server::desktop_dialogs::QueuedDialogProvider::new_with_all(
