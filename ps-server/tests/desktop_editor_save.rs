...
fn editor_json() -> String {
    ...
    ps_core::convert::sav_to_json_string(&sav_bytes).expect("sav decodes to json")
}
...
async fn save_edited_sav_writes_sav_and_confirms() {
    ...
    let round_trip = ps_core::convert::sav_to_json_string(&written).expect("written sav decodes");
