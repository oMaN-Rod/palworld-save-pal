...
async fn canceled_dialog_emits_no_file_selected() {
    let server = common::start_desktop_test_server(std::sync::Arc::new(
        ps_server::desktop_dialogs::QueuedDialogProvider::new(vec![None]),
    ))
    .await;
...
async fn wrong_filename_emits_error_with_exact_message() {
    ...
    let server = common::start_desktop_test_server(std::sync::Arc::new(
        ps_server::desktop_dialogs::QueuedDialogProvider::new(vec![Some(picked)]),
    ))
    .await;
...
async fn valid_pick_persists_save_dir_before_loading() {
    ...
    let server = common::start_desktop_test_server(std::sync::Arc::new(
        ps_server::desktop_dialogs::QueuedDialogProvider::new(vec![Some(level_sav)]),
    ))
    .await;
    ...
    let pool = ps_db::open(&db_path).await.expect("open db");
    let driver = ps_db::SqlxSqliteDriver::new(pool);
    let persisted = ps_db::settings::saved_save_dir(&driver).await.expect("read");
