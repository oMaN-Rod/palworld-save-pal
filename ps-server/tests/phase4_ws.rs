...
//!
//! `PS_GAMEPASS_PACKAGES_ROOT` and `PS_BACKUPS_ROOT` are process-global env
...
async fn gamepass_scan_and_management_flow() {
    ...
    let save = ps_core::gamepass::fixture::SyntheticSave {
        ...
        local_data: None,
        world_option: None,
        players: vec![ps_core::gamepass::fixture::SyntheticPlayer {
            id: player_id,
            sav: b"P".to_vec(),
            dps: Some(b"D".to_vec()),
        }],
    ...
    ps_core::gamepass::fixture::build_wgs_tree(gamepass_root.path(), &[save]).unwrap();
    let _env_guard = common::GamepassEnvGuard::acquire(&[
        (
            "PS_GAMEPASS_PACKAGES_ROOT",
            gamepass_root.path().to_path_buf(),
        ),
        ("PS_BACKUPS_ROOT", gamepass_root.path().join("backups")),
    ])
    .await;
...
async fn select_gamepass_save_loads_and_saves_modded_copy() {
    ...
    let pool = ps_db::open(&server._temp_dir.path().join("ps-rs.db"))
        .await
        .unwrap();
    let db = ps_db::SqlxSqliteDriver::new(pool.clone());
    ps_db::settings::update_save_dir(&db, &container_dir.to_string_lossy())
        .await
        .unwrap();
    pool.close().await;
    ...
    let saves = ps_core::gamepass::scan::scan_saves(&container_dir).unwrap();
...
/// writes a backup, so `PS_BACKUPS_ROOT` is pointed at a temp dir to keep the
...
async fn convert_save_format_standalone_round_trip_over_ws() {
    ...
    let container_dir = ps_core::gamepass::fixture::build_wgs_tree(temp.path(), &[save]).unwrap();
    let steam_out = temp.path().join("steam-out");

    let _env_guard =
        common::GamepassEnvGuard::acquire(&[("PS_BACKUPS_ROOT", temp.path().join("backups"))])
            .await;
...
fn mask_bytes(local_data_sav: &[u8]) -> Vec<u8> {
    let save = ps_core::ue::SaveReader::new()
        .game::<ps_core::ue::Palworld>()
        .types(ps_core::ue::games::palworld::palworld_types())
        .read(std::io::Cursor::new(local_data_sav))
        .unwrap();
    let ps_core::ue::Property::Struct(ps_core::ue::StructValue::Struct(save_data)) =
        &save.root.properties.0[&ps_core::ue::PropertyKey::from("SaveData")]
    else {
        panic!("SaveData missing");
    };
    let ps_core::ue::Property::Array(ps_core::ue::ValueVec::Byte(ps_core::ue::ByteArray::Byte(bytes))) =
        &save_data.0[&ps_core::ue::PropertyKey::from("WorldMapMaskTextureV4")]
    else {
...
async fn unlock_map_zeroes_mask_and_backs_up() {
    ...
    let mut save = ps_core::savio::read_sav_bytes(&meta_bytes).unwrap();
    {
        let save_data = ps_core::props::get_mut(&mut save.root.properties, &["SaveData"])
            .expect("world1 LevelMeta.sav must carry a SaveData struct");
        let save_data =
            ps_core::props::struct_props_mut(save_data).expect("SaveData must be a struct");
        save_data.insert(
            "WorldMapMaskTextureV4",
            ps_core::ue::Property::Array(ps_core::ue::ValueVec::Byte(ps_core::ue::ByteArray::Byte(vec![
                1, 2, 3, 0, 4,
            ]))),
        );
    }
    ps_core::props::ensure_schema(
        &mut save,
        "SaveData.WorldMapMaskTextureV4".to_string(),
        ps_core::ue::PropertyTagPartial {
            id: None,
            data: ps_core::ue::PropertyTagDataPartial::Array(Box::new(
                ps_core::ue::PropertyTagDataPartial::Byte(None),
            )),
        },
    );
    let grafted_local_data = ps_core::savio::write_sav_bytes(&save).unwrap();
