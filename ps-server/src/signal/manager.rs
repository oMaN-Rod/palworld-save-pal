...
const PAIRING_URL_BASE_DEFAULT: &str = "https://palstudio.app";
const PAIRING_URL_BASE_ENV: &str = "PS_SIGNAL_PAIRING_URL_BASE";
...
const MEET_RELINK_BACKOFF_ENV: &str = "PS_SIGNAL_MEET_RELINK_BACKOFF_MS";
...
const DEVICE_CREDENTIAL_ACK_TIMEOUT_ENV: &str = "PS_SIGNAL_DEVICE_ACK_TIMEOUT_MS";
const DEFAULT_DEVICE_NAME: &str = "Paired device";
const FALLBACK_DESKTOP_NAME: &str = "PalStudio";
...
impl SignalManager {
    ...
    pub async fn set_source(
        ...
        match selection {
            ...
            SourceSelection::Server { server_id } => {
                let record = ps_db::servers::get_server(&*state.driver, server_id)
                    .await
    ...
    pub async fn set_armed(&mut self, armed: bool, state: &Arc<AppState>) -> Result<(), String> {
        ...
        if let Err(error) = ps_db::meta::set(&*state.driver, ARMED_KEY, flag).await {
            if changed {
    ...
    pub async fn restore_armed(&mut self, state: &Arc<AppState>) -> Result<(), String> {
        let stored = ps_db::meta::get(&*state.driver, ARMED_KEY)
            .await
            .map_err(|error| error.to_string())?;
    ...
    pub async fn reset_remote_access(&mut self, state: &Arc<AppState>) -> Result<(), String> {
        ...
        ps_db::signal_devices::delete_all_devices(&*state.driver)
            .await
            .map_err(|error| error.to_string())?;
        ps_db::meta::set(
            &*state.driver,
...
async fn load_or_mint_identity(db: &dyn ps_db::DbDriver) -> Result<[u8; 32], String> {
    let stored = ps_db::meta::get(db, IDENTITY_SECRET_KEY)
        .await
        .map_err(|error| error.to_string())?;
    ...
    ps_db::meta::set(db, IDENTITY_SECRET_KEY, &hex32(&secret))
        .await
        .map_err(|error| error.to_string())?;
    Ok(secret)
}

async fn device_seal_key(db: &dyn ps_db::DbDriver, device_id: &str) -> Option<[u8; 32]> {
    let devices = ps_db::signal_devices::list_devices(db).await.ok()?;
    let device = devices
        .into_iter()
...
async fn device_name(db: &dyn ps_db::DbDriver, device_id: &str) -> Option<String> {
    let devices = ps_db::signal_devices::list_devices(db).await.ok()?;
    devices
        .into_iter()
        .find(|device| device.device_id == device_id)
...
async fn run_credential_mint(
    ...
    let device = ps_db::signal_devices::SignalDevice {
        device_id,
    ...
    match ps_db::signal_devices::insert_device(&*driver, &device).await {
        Ok(()) => {
            ...
            event = events.recv() => {
                ...
                match event {
                    PeerEvent::Connected => {
                        ...
                        if let Err(error) =
                            ps_db::signal_devices::touch_device(&*driver, &device_id, now_ms()).await
                        {
...
mod tests {
    ...
    async fn test_driver_arc() -> Arc<dyn ps_db::DbDriver> {
        Arc::new(test_driver().await)
    }

    fn device_row(device_id: &str, secret: &[u8; 32]) -> ps_db::signal_devices::SignalDevice {
    ...
    fn the_pairing_url_carries_the_ungrouped_code_in_the_fragment() {
        ...
        assert_eq!(url, format!("https://palstudio.app/signal#v1.{code}"));
    ...
    async fn a_device_key_is_found_only_while_the_device_is_paired() {
        ...
        ps_db::signal_devices::insert_device(&db, &device_row("dev-1", &DEVICE_SECRET))
            .await
            .unwrap();
        ...
        ps_db::signal_devices::delete_device(&db, "dev-1")
            .await
            .unwrap();
    ...
    async fn the_identity_secret_is_minted_once_and_kept() {
        ...
        assert_eq!(
            ps_db::meta::get(&db, IDENTITY_SECRET_KEY).await.unwrap(),
            Some(hex32(&first))
        );
    }
    ...
    async fn credential_mint_persists_nothing_without_an_ack() {
        ...
        assert!(ps_db::signal_devices::list_devices(&*driver)
            .await
            .unwrap()
            .is_empty());
    }
    ...
    async fn a_connected_device_session_touches_last_seen_and_publishes_the_connected_device() {
        ...
        ps_db::signal_devices::insert_device(&db, &device_row("dev-1", &DEVICE_SECRET))
            .await
            .unwrap();
        let driver: Arc<dyn ps_db::DbDriver> = Arc::new(db);
        let (pairing, _rx) = watch::channel(PairingState::Off);
        let (events, events_rx) = mpsc::channel(4);
