...
async fn read_snapshot(driver: &dyn ps_db::DbDriver) -> Option<Snapshot> {
...
async fn read_stored_active_id(driver: &dyn ps_db::DbDriver) -> Option<String> {
    match ps_db::meta::get(driver, ACTIVE_INSTANCE_KEY).await {
...
pub async fn resolve_active_target(driver: &dyn ps_db::DbDriver) -> Option<BridgeTarget> {
...
/// starts after PalStudio, or restarts under a new pid, gets adopted without
...
pub async fn reconcile_active_target(driver: &dyn ps_db::DbDriver, bridge: &BridgeService) {
...
pub async fn handle_game_delete_instance(
    ...
    if active_row_id(services) == Some(row) {
        ...
        if let Err(error) = ps_db::meta::set(&*ctx.app.driver, ACTIVE_INSTANCE_KEY, "").await {
            tracing::warn!(
                %error,
...
mod tests {
    ...
    impl ps_db::DbDriver for FailingDriver {
        async fn execute(
            &self,
            _sql: &str,
            _params: &[ps_db::DbValue],
        ) -> Result<u64, ps_db::DbError> {
            Err(ps_db::DbError::Other("simulated failure".to_string()))
        }
        ...
        async fn query(
            ...
            _sql: &str,
            _params: &[ps_db::DbValue],
        ) -> Result<Vec<ps_db::DbRow>, ps_db::DbError> {
            Err(ps_db::DbError::Other("simulated failure".to_string()))
        }
    }
    ...
    /// Serializes tests in this module that mutate `PS_BRIDGE_ENDPOINT_DIR`
    ...
    impl BridgeEndpointEnvGuard {
        async fn acquire(dir: &std::path::Path) -> Self {
            ...
            let previous = std::env::var_os("PS_BRIDGE_ENDPOINT_DIR");
            std::env::set_var("PS_BRIDGE_ENDPOINT_DIR", dir);
            Self {
                _lock: lock,
                previous,
            }
    ...
    impl Drop for BridgeEndpointEnvGuard {
        fn drop(&mut self) {
            match &self.previous {
                Some(value) => std::env::set_var("PS_BRIDGE_ENDPOINT_DIR", value),
                None => std::env::remove_var("PS_BRIDGE_ENDPOINT_DIR"),
            }
        }
    }
