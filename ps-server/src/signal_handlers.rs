...
use ps_app::live::SignalSourceStatus;
...
const PAIRING_GATE_MESSAGE: &str =
    "Pairing must be started from PalStudio on the machine it is running on.";
...
fn device_entry(
    device: &ps_db::signal_devices::SignalDevice,
...
async fn device_list_payload(
    ...
    let devices = ps_db::signal_devices::list_devices(&*ctx.app.driver)
        .await
...
pub async fn handle_signal_rename_device(
    ...
    match ps_db::signal_devices::rename_device(&*ctx.app.driver, &payload.device_id, name).await {
        Ok(false) => refuse(
            ctx.emitter,
            MessageType::SignalRenameDevice,
...
pub async fn handle_signal_revoke_device(
    ...
    match ps_db::signal_devices::delete_device(&*ctx.app.driver, &payload.device_id).await {
        Ok(false) => refuse(
            ctx.emitter,
...
mod tests {
    ...
    fn paired_device(device_id: &str) -> ps_db::signal_devices::SignalDevice {
    ...
    async fn remote_access_control_from_a_non_loopback_connection_is_refused() {
        ...
        ps_db::signal_devices::insert_device(&*driver, &paired_device("dev-1"))
            .await
            .unwrap();
