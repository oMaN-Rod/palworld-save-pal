...
/// `app_root` is the writable base dir — the desktop shell exports `PS_APP_ROOT`
...
pub fn folder_path_for(folder_type: &str, app_root: &Path) -> Option<PathBuf> {
    match folder_type {
        ...
        "ps_root" => Some(app_root.to_path_buf()),
        _ => None,
    }
}
...
pub async fn handle_open_folder(
    ...
    let app_root = ps_core::paths::app_root();
...
mod tests {
    ...
    fn is_openable_url_accepts_only_http_schemes() {
        ...
        assert!(is_openable_url("https://github.com/oMaN-Rod/palstudio"));
...
mod desktop_system_tests {
    ...
    fn folder_path_resolves_all_four_python_folder_types() {
        ...
        assert_eq!(
            folder_path_for("ps_root", app_root),
