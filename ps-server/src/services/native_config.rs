...
/// tuple an older PalStudio left open ends at the next `Key=` instead of swallowing
...
fn option_settings_close_offset(body: &str) -> Option<usize> {
    ...
    // Nothing balances when an older PalStudio left a tuple open, but the list still
    // ends at the last `)` on the line.
    let line_end = body.find('\n').unwrap_or(body.len());
    body[..line_end].rfind(')')
}

/// Closes a tuple an older PalStudio left open so the healed value goes back to disk.
...
mod tests {
    ...
    fn native_record(install_path: &str) -> ps_db::servers::ServerRecord {
    ...
    /// The fallback defaults are the inventory of settings PalStudio claims to know,
    ...
    /// PalStudio 1.3.3 and earlier truncated the list at the first `)`, so every ini
    ...
    /// The end of the reported 1.4.0 import regression: a server whose ini PalStudio
    /// itself mangled came in with a blank name and stock player count.
    #[test]
    fn import_reads_an_ini_written_by_an_older_ps() {
        let scratch = tempfile::tempdir().unwrap();
        let install = scratch.path().to_string_lossy().to_string();
        let cfg = config_dir(&install);
        std::fs::create_dir_all(&cfg).unwrap();
    ...
    #[test]
    fn build_content_closes_a_tuple_an_older_ps_left_open() {
        let scratch = tempfile::tempdir().unwrap();
        let install = scratch.path().to_string_lossy().to_string();
        let cfg = config_dir(&install);
        std::fs::create_dir_all(&cfg).unwrap();
