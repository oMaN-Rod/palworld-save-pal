...
//! workspace: the generated `ps.lua` annotations, a `.luarc.json`, and one
//! file per source key.

use std::collections::{BTreeMap, HashMap, HashSet};
use std::path::{Path, PathBuf};

const META_FILE: &str = "ps.lua";
const LUARC_FILE: &str = ".luarc.json";

/// `inject-field` and `missing-fields` are disabled because the generated
/// `ps.lua` trips 25 of them against itself — a file the plugin author did
...
const LUARC: &str = r#"{
  ...
  "workspace": { "checkThirdParty": false, "library": ["ps.lua"] }
}
"#;
...
pub fn materialise(
    ...
    write_file(
        ...
        &ps_plugin::lua_meta(&ps_plugin::api_definition()),
    )?;
...
fn check_no_case_collision(sources: &BTreeMap<String, String>) -> Result<(), String> {
    ...
    // `ps.lua` is what `.luarc.json` names as the workspace library: an
    // author's own `ps.lua` would replace the whole host API annotation set
    // and turn every `ps.*` call in their plugin into a false diagnostic.
...
mod tests {
    ...
    fn a_source_named_like_a_generated_file_is_refused() {
        ...
        nested.insert("ps.lua/x.lua".to_string(), "return {}".to_string());
    ...
    fn the_generated_meta_file_is_the_one_that_survives() {
        ...
        let workspace =
            materialise(dir.path(), "user.demo", &one("main.lua")).expect("materialise");
        assert_eq!(
            std::fs::read_to_string(workspace.join(META_FILE)).expect("ps.lua"),
            ps_plugin::lua_meta(&ps_plugin::api_definition())
        );
    }
