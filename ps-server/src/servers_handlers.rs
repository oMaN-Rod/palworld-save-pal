...
pub async fn handle_list_servers(
    ...
    match ps_db::servers::list_servers(&*ctx.app.driver).await {
        Ok(records) => {
...
async fn create_server_impl(
    ...
    if is_native {
        ...
        emit_creation_progress(emitter, "Validating server configuration...");
        let base_path = ps_core::paths::app_root()
            .join("servers")
...
async fn update_server_impl(
    ...
    let Some(mut record) = ps_db::servers::update_server(db, data.server_id, &data.updates)
        .await
        .map_err(|error| error.to_string())?
    else {
