...
describe('credentialStore', () => {
	...
	it('returns null and removes a corrupt (non-JSON) entry', () => {
		localStorage.setItem('ps-signal-device', 'not json{{{');

		expect(loadStoredDesktop()).toBeNull();
		expect(localStorage.getItem('ps-signal-device')).toBeNull();
	});

	it('returns null and removes an entry missing a required field', () => {
		localStorage.setItem('ps-signal-device', JSON.stringify({ ...SAMPLE, deviceId: undefined }));

		expect(loadStoredDesktop()).toBeNull();
		expect(localStorage.getItem('ps-signal-device')).toBeNull();
	});

	it('stores under the ps-signal-device key', () => {
		saveStoredDesktop(SAMPLE);
		expect(localStorage.getItem('ps-signal-device')).toBe(JSON.stringify(SAMPLE));
	});
