...
export function storageWarning(
	...
	return bridge.opfsSupported
		? 'Another tab has PalStudio open and is using the database; presets, blueprints and stored pals will not be saved from this tab. Close the other tab and reload.'
	...
	for (let attempt = 1; attempt <= poolAttempts; attempt++) {
		try {
			...
			if (attempt < poolAttempts) {
				console.warn(`[ps] OPFS SAH pool busy (attempt ${attempt}/${poolAttempts}); retrying`);
...
export async function openSqlite(options: OpenSqliteOptions = {}): Promise<SqliteBridge> {
	try {
		...
		console.error('[ps] sqlite failed to initialise; database features are disabled:', e);
