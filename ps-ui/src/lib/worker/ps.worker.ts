...
export async function gvasFilesToSavZip(
	...
	return { name: `${worldName || 'PalStudio'}.zip`, zip: zipSync(entries) };
}
...
if (typeof WorkerGlobalScope !== 'undefined' && self instanceof (WorkerGlobalScope as never)) {
	let wasmReady: Promise<typeof import('$lib/wasm/ps')> | null = null;
	async function wasm() {
		if (!wasmReady) {
			wasmReady = (async () => {
				const mod = await import('$lib/wasm/ps');
				await mod.default();
				mod.init();
				mod.set_emit_callback((frame: string) => self.postMessage(frame));
				// The engine links no Oodle codec on wasm32; without this every
				// `.sav`/`.psbp` it writes for itself — blueprints, above all —
