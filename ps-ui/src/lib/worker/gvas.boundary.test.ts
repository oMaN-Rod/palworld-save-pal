...
describe.skipIf(!hasGolden)('GVAS boundary: ooz.wasm ↔ ps_core', () => {
	let sav: Uint8Array;
	let golden: Uint8Array;
	beforeAll(() => {
		sav = new Uint8Array(readFileSync(savPath));
		golden = new Uint8Array(readFileSync(goldenPath)); // run `cargo test -p ps-core --test gvas_boundary` first
	});

	it('ooz-decompressed real Level.sav equals ps_core write_gvas_bytes output', async () => {
