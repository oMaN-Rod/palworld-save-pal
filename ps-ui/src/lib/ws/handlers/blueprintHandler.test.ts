...
describe('handleExportFrame', () => {
	it('downloads each file when the frame is a browser array', () => {
		...
		const toast = vi.fn();
		handleExportFrame([{ name: 'Home.ps', content: 'QUJD' }], { download, toast });
		expect(download).toHaveBeenCalledWith('Home.ps', 'QUJD');
	...
	it('toasts success when the frame is a desktop write result', () => {
		...
		const toast = vi.fn();
		handleExportFrame(
			{ message: 'Blueprint Home exported', file_path: '/x/Home.ps' },
			{ download, toast }
		);
