...
<script lang="ts">
	...
	const splitOrientation = persistedState<SplitOrientation>(
		'ps-plugin-editor-split-orientation',
		'horizontal'
	);
	const splitRatio = persistedState<number>('ps-plugin-editor-split-ratio', DEFAULT_SPLIT_RATIO);
	...
	$effect(() => {
		...
		monaco.editor.setModelMarkers(model, 'ps', markers);
	});
	...
	$effect(() => {
		...
		monaco.editor.setModelMarkers(model, 'ps-lsp', markers);
	});
