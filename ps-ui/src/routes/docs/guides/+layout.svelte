<script lang="ts">
	...
	function onProseClick(e: MouseEvent) {
		const img = (e.target as HTMLElement).closest('.prose-ps img') as HTMLImageElement | null;
		if (img) {
			lightboxSrc = img.src;
			lightboxAlt = img.alt || '';
		}
...
<div class="flex h-full overflow-hidden">
	...
	<main class="prose-ps min-w-0 flex-1 overflow-y-auto p-6 bg-surface-900/25" onclick={onProseClick}>
		{@render children()}
	</main>
	{#if showToc}
