...
PalStudio can stream a live view of your world to a phone or tablet — pair a device once, then watch the map update in real time while you're away from your PC.

## What Signal Is {.toc}

**Signal** is the pairing and live-view system built into PalStudio. It has three parts:
...
By default, pairing and remote-access controls can only be started from the same machine the app is running on (loopback only) — a safeguard against turning brief LAN reach into durable remote access. If you're self-hosting PalStudio on a box you administer from another machine (a home server, a container host, etc.), set:

```
PS_SIGNAL_ALLOW_REMOTE_PAIRING=1
```

on the machine running PalStudio to allow pairing and remote-access controls from a non-loopback connection. Only set this on a host you trust to reach that machine's admin surface.
