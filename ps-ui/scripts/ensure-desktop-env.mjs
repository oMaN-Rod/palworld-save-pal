// Writes ps-ui/.env for desktop builds. Default mode repairs the file only when it
...
/**
 * @param {string | null} existing contents of ps-ui/.env, or null when absent
...
/**
 * The build target, read from the same ps-ui/.env that `$env/static/public` serves
 ...
 *
 * @param {string | null} contents contents of ps-ui/.env, or null when absent
 */
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
	...
	if (desktopEnvNeedsWrite(existing, { force })) {
		...
		console.log(`[ensure-desktop-env] wrote ps-ui/.env for desktop${force ? ' (forced)' : ''}`);
	} else {
		console.log('[ensure-desktop-env] ps-ui/.env already set for desktop, leaving it alone');
	}
}
