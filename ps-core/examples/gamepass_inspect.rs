...
//!
//! Usage: cargo run -p ps-core --example gamepass_inspect -- <container_dir>
//!            [--save <id>] [--all] [--deep] [--quiet-blobs]

use std::collections::{BTreeMap, BTreeSet};
use std::path::{Path, PathBuf};

use ps_core::gamepass::format::{guid_file_name, Filetime};
...
fn describe_payload(data: &[u8], deep: bool) -> String {
    ...
    let decompressed = ps_core::ue::compression::decompress_save(&mut std::io::Cursor::new(data));
    let decode = match &decompressed {
        Ok(gvas) => format!("decompress OK ({} bytes GVAS)", gvas.len()),
        Err(error) => format!("DECOMPRESS FAILED: {error}"),
    };
    let parse = if deep {
        match ps_core::savio::read_sav_bytes(data) {
...
/// What ps-core itself makes of the directory. The lenient parse above exists to
...
struct AppView {
    index: Option<ps_core::gamepass::format::ContainerIndex>,
...
impl AppView {
    fn load(container_dir: &Path) -> Self {
        AppView {
            index: ps_core::gamepass::format::ContainerIndex::read_from_dir(container_dir).ok(),
            listed: ps_core::gamepass::scan::scan_saves(container_dir)
                .map(|saves| {
                    saves
                        .iter()
...
/// candidates. Falls back to the lenient replica only when ps-core cannot read the
...
fn print_scan_verdict(
    ...
    for (key, (id, name)) in selection {
        ...
        match ps_core::gamepass::store::read_first_blob(container_dir, &entry) {
            Ok(Some((seq, data))) => {
...
fn find_entry(
    ...
    id: &uuid::Uuid,
    name: &str,
) -> Option<ps_core::gamepass::format::ContainerEntry> {
    app.index
        .as_ref()?
        .containers
        .iter()
