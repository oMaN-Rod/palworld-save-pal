...
use ps_core::dto::container::DynamicItemDto;
use ps_core::dto::ordered_map::OrderedMap;
use ps_core::dto::pal::PalGender;
...
fn written_level_parses_back(session: &SaveSession, what: &str) {
    ...
    if let Err(e) = ps_core::savio::read_sav_bytes(&bytes) {
...
fn raw_schema_paths(fixture: &str, file: &str) -> Vec<String> {
    ...
    let save = ps_core::savio::read_sav_bytes(&bytes).expect("parse fixture save");
    save.schemas.schemas().keys().cloned().collect()
}

fn loaded_player_ids(
    session: &mut ps_core::session::SaveSession,
    data: &GameData,
) -> Vec<uuid::Uuid> {
...
fn adding_a_dps_pal_serializes_when_the_dps_slots_are_empty() {
    ...
    ps_core::savio::read_sav_bytes(dps).expect("_dps.sav must parse back after adding a pal");
}
