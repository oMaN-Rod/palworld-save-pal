...
use ps_core::domain::{pal, world};
use ps_core::gamedata::GameData;
use ps_core::session::{SaveKind, SaveSession};
use ps_core::ue::games::palworld::PalCharacterData;
use ps_core::ue::{
...
fn every_corpus_pal_reads_into_a_dto() {
    ...
    for entry in entries {
        ...
        assert_eq!(
            dto.character_key,
            ps_core::dto::pal::format_character_key(&dto.character_id, pal::known_pal_keys(&data))
        );
        assert!(dto.level >= 1);
        pal_count += 1;
    }
...
fn world1_fixture_base_camp_worker_director_decodes_to_known_real_values() {
    ...
    let (guild_id, container_id) =
        ps_core::domain::guild::base_guild_and_container(&bases[0]).expect("decodes cleanly");
...
fn slot_key_name_counts(level_sav: &std::path::Path) -> (usize, usize) {
    ...
    let gvas =
        ps_core::ue::compression::decompress_save(&mut std::io::Cursor::new(bytes.as_slice()))
...
fn resolved_slot_key_counts(session: &SaveSession) -> (usize, usize) {
    ...
    for entry in world::character_map(&session.level).expect("character map") {
        ...
        if ps_core::props::get(save_parameter, &["SlotID"]).is_some() {
            counts.0 += 1;
        }
        if ps_core::props::get(save_parameter, &["SlotId"]).is_some() {
            counts.1 += 1;
...
fn the_reference_save_spells_the_pal_slot_key_slot_id_uppercase() {
    let level_sav = ps_core::gamepass::fixture::reference_saves_dir().join("Level.sav");
...
fn pal_dto_from_entry_returns_none_for_a_malformed_entry() {
    let malformed = ps_core::ue::MapEntry {
        key: ps_core::ue::Property::Bool(true),
        value: ps_core::ue::Property::Bool(true),
    };
    let data = game_data();
...
fn read_save_parameter_dto_applies_every_default_for_an_empty_save_parameter() {
    let empty_save_parameter = ps_core::ue::Properties::default();
    let data = game_data();
    ...
    assert_eq!(dto.storage_id, ps_core::props::EMPTY_UUID);
