...
use ps_core::ue::{Property, StructValue};
...
fn dynamic_item_index_resolves_every_real_entry_by_local_id_in_created_world() {
    ...
    for (&local_id, &position) in &index {
        ...
        let Some(Property::Struct(StructValue::Game(ps_core::ue::PalStruct::DynamicItem(dynamic_item)))) =
            ps_core::props::get(item_props, &["RawData"])
        else {
            panic!("indexed position {position} has no PalDynamicItem RawData");
        };
        assert_eq!(
            local_id,
            ps_core::props::guid_to_uuid(&dynamic_item.id.local_id_in_created_world),
