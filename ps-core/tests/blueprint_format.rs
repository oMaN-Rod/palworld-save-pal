...
use ps_core::domain::blueprint::{BaseBlueprint, BlueprintHeader, CaptureOptions, SCHEMA_VERSION};
use ps_core::domain::blueprint::{capture, gvas};
use ps_core::session::SaveSession;
use ps_core::ue::games::palworld::PalTransform;
use ps_core::ue::{
...
fn entry_raw_data(entry: &MapEntry) -> Option<&StructValue> {
    ps_core::props::struct_props(&entry.value).and_then(raw_data_of)
}
...
fn json_round_trip_preserves_property_variants_not_just_values() {
    ...
    for (a, b) in original.structures.iter().zip(restored.structures.iter()) {
        ...
        assert!(
            ps_core::domain::blueprint::capture::map_object_id_is_name_property(&b.properties),
...
fn json_round_trip_survives_a_re_encode_to_gvas_bytes() {
    ...
    let direct = gvas::to_psbp_bytes(&original).expect("psbp encode");
    let via_json = gvas::from_json(&gvas::to_json(&original).expect("json encode"))
        .expect("json decode");
    let round_tripped = gvas::to_psbp_bytes(&via_json).expect("psbp encode after json");
...
#[test]
fn ps_round_trip_preserves_the_blueprint() {
    let session = common::load_fixture_session("v1_relics");
    let base_id = common::fixture_base_id(&session);
    let original =
        capture::capture(&session, base_id, CaptureOptions::full(), "Home").expect("capture");

    let bytes = gvas::to_psbp_bytes(&original).expect("blueprint must serialize to psbp bytes");
    let restored = gvas::from_psbp_bytes(&bytes).expect("psbp bytes must parse back");
...
#[test]
fn ps_and_json_encodings_agree() {
    let original = captured_fixture_blueprint();

    let from_psbp =
        gvas::from_psbp_bytes(&gvas::to_psbp_bytes(&original).expect("psbp encode")).expect("psbp decode");
    let from_json =
        gvas::from_json(&gvas::to_json(&original).expect("json encode")).expect("json decode");

    assert_eq!(from_psbp.structures.len(), from_json.structures.len());
    assert_eq!(from_psbp.header.manifest, from_json.header.manifest);
    // Lengths and the manifest alone can agree even while `.psbp` decodes every
    // payload as an opaque byte array; the type view is what actually differs.
    assert_eq!(
        typed_counts(&from_psbp),
...
#[test]
fn a_ps_round_trip_keeps_every_typed_palworld_struct() {
    let original = captured_fixture_blueprint();
    ...
    assert_eq!(typed_counts(&from_psbp), expected, "psbp decode must preserve typed structs");

    assert!(
        capture::first_build_player_uid(&from_psbp).is_some(),
        "a ps-decoded blueprint must still expose its typed Model.RawData"
    );
    assert_eq!(
        capture::first_build_player_uid(&from_psbp),
...
fn a_labelled_byte_property_survives_both_encodings() {
    ...
    original.structures[0]
        .properties
        .insert("PsLabelledByte", Property::Byte(Byte::Label("EPal::Foo".to_string())));
    ...
    for (label, restored) in [("psbp", &from_psbp), ("json", &from_json)] {
        let value = restored.structures[0]
            .properties
            .0
            .get(&PropertyKey::from("PsLabelledByte"));
        assert!(
...
fn two_irreconcilable_types_at_one_path_are_refused_at_encode_time() {
    ...
    blueprint.structures[0]
        .properties
        .insert("PsCollision", Property::Name("AAA".to_string()));
    blueprint.structures[1]
        .properties
        .insert("PsCollision", Property::Str("BBB".to_string()));

    let message = gvas::to_psbp_bytes(&blueprint)
        .expect_err("a name/str collision at one path must not encode")
        .to_string();

    assert!(message.contains("PsCollision"), "the error must name the path, got: {message}");
    assert!(
...
fn json_from_a_save_that_is_not_a_blueprint_is_refused() {
    ...
    assert!(
        message.contains("PsBaseBlueprint"),
...
fn quat_parts(q: &ps_core::ue::Quat) -> (f64, f64, f64, f64) {
...
fn placed_world_transforms(session: &SaveSession, base_id: uuid::Uuid) -> Vec<PalTransform> {
    let Ok(Some(values)) = ps_core::domain::world::map_object_values(&session.level) else {
        return Vec::new();
    };
    values
        .iter()
        .filter_map(|value| {
            ...
            match model.0.get(&PropertyKey::from("RawData")) {
                Some(Property::Struct(StructValue::Game(PalStruct::MapModel(raw)))) => {
                    (ps_core::props::guid_to_uuid(&raw.base_camp_id_belong_to) == base_id)
...
/// Comparing encoded sizes alone is not enough: a `.psbp` decode that produced zero
...
fn a_blueprint_decoded_from_either_encoding_still_places() {
    ...
    for (label, decoded) in [
        (
            "psbp",
            gvas::from_psbp_bytes(&gvas::to_psbp_bytes(&original).expect("psbp encode"))
                .expect("psbp decode"),
        ),
        (
            "json",
        ...
        assert_eq!(
            ps_core::domain::guild::base_structures(&reloaded, new_base_id).len(),
            FIXTURE_BASE_STRUCTURES,
