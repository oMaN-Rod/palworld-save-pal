...
use ps_core::domain::{containers, pal, player, world};
use ps_core::dto::pal::PalDto;
use ps_core::error::CoreError;
use ps_core::gamedata::GameData;
use ps_core::progress::null_progress;
use ps_core::session::{LoadedPlayer, SaveKind, SaveSession};
use ps_core::ue::{
...
fn guid_property(id: Uuid) -> Property {
    Property::Struct(StructValue::Guid(ps_core::props::uuid_to_guid(id)))
}
...
// world1 has no `_dps.sav` for either player, so the DPS ops further down are
// exercised synthetically instead.
...
fn add_and_delete_player_pal_invalidate_caches_and_shift_the_rebuilt_index() {
    ...
    // Not `entries[0]`: that is often the player's own character entry, which
    // `delete_player_pals`'s ownership guard rightly rejects.
            ...
            .find(|entry| {
                !world::entry_is_player(entry)
                    && world::entry_save_parameter(entry).and_then(|params| {
                        ps_core::props::get(params, &["OwnerPlayerUId"])
                            .and_then(ps_core::props::as_uuid)
                    }) == Some(player_id)
            })
...
fn add_player_pal_from_dto_writes_the_target_player_as_owner_when_the_source_owner_was_nil() {
    ...
    if let Some(save_parameter) = gps_slot_props
        ...
        .and_then(ps_core::props::struct_props_mut)
    {
        save_parameter.insert("OwnerPlayerUId", guid_property(ps_core::props::EMPTY_UUID));
    }
    ...
    let raw_owner = world::character_map(&session.level)
        ...
        .and_then(|save_parameter| {
            ps_core::props::get(save_parameter, &["OwnerPlayerUId"])
                .and_then(ps_core::props::as_uuid)
        });
    assert_eq!(
        raw_owner,
...
fn clone_pal_matches_source_stats_when_the_pal_box_has_room() {
    ...
    match pal::clone_pal(&mut session, &data, &source).unwrap() {
        ...
        None => {
            ...
            // (which `clone_pal` misreads as full; see the deterministic test below).
        }
    }
}
...
/// Without the ownership guard, `delete_pal_entry`'s unscoped whole-map search would
/// delete player B's pal from the save when called through player A.
#[test]
...
/// Unlike `clone_pal` (see `clone_pal_at_slot_zero_...` below), `add_guild_pal` accepts
/// slot index 0: the base's worker container starts with its only slot empty.
#[test]
fn add_guild_pal_at_slot_zero_succeeds_and_leaves_owner_player_uid_present() {
    ...
    ps_core::domain::guild::get_guild_details(&mut session, &data, guild_id)
        .unwrap()
        .expect("guild loads");
    ...
    let (_, worker_container_id) =
        ps_core::domain::guild::base_guild_and_container(base_camp).unwrap();
    ...
    assert_eq!(raw_owner, Some(ps_core::props::EMPTY_UUID));
    assert!(
...
// world1's only base has a single slot and no second guild with a base, so nothing
...
fn zero_transform() -> ps_core::ue::games::palworld::PalTransform {
    use ps_core::ue::{Double, Quat, Vector};
    ps_core::ue::games::palworld::PalTransform {
        rotation: Quat {
            x: Double(0.0),
            y: Double(0.0),
            z: Double(0.0),
...
fn base_camp_entry(base_id: Uuid, guild_id: Uuid, worker_container_id: Uuid) -> MapEntry {
    ...
    let camp = PalBaseCamp {
        ...
        area_range: 0.0,
        group_id_belong_to: ps_core::props::uuid_to_guid(guild_id),
        fast_travel_local_transform: zero_transform(),
        owner_map_object_instance_id: ps_core::ue::FGuid::nil(),
        trailing_bytes: [0; 4],
    };
    ...
    value_properties.insert(
        "RawData",
        Property::Struct(StructValue::Game(ps_core::ue::PalStruct::BaseCamp(Box::new(camp)))),
    );
    value_properties.insert(
        "WorkerDirector",
...
fn guild_group_entry(guild_id: Uuid) -> MapEntry {
    use ps_core::ue::games::palworld::PalGroupData;
    let mut value_properties = Properties::default();
    value_properties.insert(
        "GroupType",
    ...
    let group_data = PalGroupData {
        group_id: ps_core::props::uuid_to_guid(guild_id),
        group_name: String::new(),
        individual_character_handle_ids: vec![],
        data: ps_core::ue::games::palworld::PalGroupVariant::Guild(
            ps_core::domain::guild_tail::pre_update_guild(1, "", uuid::Uuid::nil(), &[]),
        ),
    };
    value_properties.insert(
        "RawData",
...
/// N independent guild/base/worker-container triples, each guild pre-loaded and each
/// base's worker container empty. Callers seed pals through the real `pal::add_guild_pal`.
...
/// Without the membership guard, `delete_pal_entry`'s unscoped whole-map search would
/// delete guild B's base pal when called through guild A.
#[test]
...
fn empty_character_container_entry(container_id: Uuid, slot_num: i32) -> MapEntry {
    ...
    let mut value_props = Properties::default();
    value_props.insert("SlotNum", ps_core::props::int_property(slot_num));
...
/// One loaded player owning one pal, with an empty (SlotNum 1) pal box -- forcing the
/// pal box's first free slot to be 0, deterministically.
...
fn clone_pal_at_slot_zero_is_reported_as_box_full() {
    ...
    // The orphaned Slots entry from the bail-out is left in place on purpose: cleaning
    // it up would diverge from the game's own on-disk result.
...
/// The pal-box mutation runs before the source-pal lookup, so a source id that
/// doesn't exist leaves the same orphaned slot behind.
#[test]
fn clone_pal_with_an_unowned_source_id_also_leaves_the_orphaned_slot() {
    ...
    // Two slots so the slot-0 bug can't fire; only "source not found" is under test.
    {
        let entries = world::character_container_map_mut(&mut session.level).unwrap();
        let value_props = ps_core::props::struct_props_mut(&mut entries[0].value).unwrap();
        value_props.insert("SlotNum", ps_core::props::int_property(2));
    }
...
fn dps_slot(character_id: &str, instance_id: Uuid) -> StructValue {
    ...
    if character_id != "None" {
        save_parameter.insert("CharacterID", ps_core::props::name_property(character_id));
    }
    save_parameter.insert("Level", ps_core::props::byte_property(5));
    save_parameter.insert("Talent_HP", ps_core::props::byte_property(30));
    ...
    slot_struct.insert("SlotIndex", ps_core::props::int_property(-1));
    ...
    save_parameter.insert(
        ...
        Property::Array(ValueVec::Struct(vec![StructValue::Struct({
            ...
            p.insert("StatusName", ps_core::props::name_property("最大HP"));
            p.insert("StatusPoint", ps_core::props::int_property(0));
            p
        })])),
    );
    save_parameter.insert(
...
/// A two-slot `SaveParameterArray`: slot 0 empty, slot 1 holding a lucky pal
/// (`IsRarePal: true`) deliberately, for the recycled-slot test below.
...
fn add_player_dps_pal_fills_the_first_empty_slot_and_computes_max_hp() {
    ...
    assert_eq!(new_pal.character_id, "Sheepball");
    assert_eq!(new_pal.owner_uid, Some(player_id));
    assert_eq!(new_pal.gender, ps_core::dto::pal::PalGender::Female);
    assert_eq!(new_pal.hp, new_pal.max_hp);
    assert!(new_pal.hp > 0);
}

/// A DPS slot reset never clears `IsRarePal`, so a slot recycled from a lucky pal hands
/// that flag to the brand-new pal created there. Preserved for save-file fidelity.
#[test]
...
/// Slot 0 is never-used (`CharacterID` "None", no `FullStomach` key). Slot 1 is
/// recycled from an "Anubis" (`max_full_stomach` 540.0) and carries a stale 999.0,
/// chosen to collide with neither the 150.0 default nor either species' real max.
fn dps_fixture_for_stomach() -> (SaveSession, GameData, Uuid) {
    ...
    if let Some(save_parameter) = recycled_slot_props
        ...
        .and_then(ps_core::props::struct_props_mut)
    {
        save_parameter.insert("FullStomach", ps_core::props::float_property(999.0));
    }
...
/// The value written keys off the slot's PREVIOUS occupant ("Anubis", 540.0), not the
...
fn delete_player_dps_pals_resets_the_slot_and_clears_the_outer_instance_id() {
    ...
    let slots_before = ps_core::props::struct_values(
        dps_save
            .root
            .properties
            .0
            .get(&ps_core::ue::PropertyKey::from("SaveParameterArray"))
            .unwrap(),
    )
    .unwrap();
    ...
    let loaded = session.loaded_players.get(&player_id).unwrap();
    let dps_save = loaded.dps.as_ref().unwrap();
    let slots_after = ps_core::props::struct_values(
        dps_save
            .root
            .properties
            .0
    ...
    let save_parameter = ps_core::props::struct_props(
        slot_props
            .0
    ...
    .unwrap();
    assert_eq!(
        ps_core::props::get(save_parameter, &["CharacterID"]).and_then(ps_core::props::as_str),
        Some("None"),
        "reset() must clear CharacterID back to \"None\""
    );
    let outer_instance_id = ps_core::props::get(slot_props, &["InstanceId", "InstanceId"])
        .and_then(ps_core::props::as_uuid)
        .unwrap();
    assert_eq!(
        outer_instance_id,
...
/// All three DPS ops must return `None` or no-op, never panic, when the player has
/// no `_dps.sav` (world1's state for every player).
#[test]
