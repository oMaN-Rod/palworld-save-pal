...
use ps_core::domain::player::{self, STATUS_NAME_MAP};
...
fn first_player_id(session: &mut ps_core::session::SaveSession) -> Uuid {
    *session
        .player_file_refs
        .keys()
        .next()
...
fn resave_level_sav(edit: impl FnOnce(&mut ps_core::dto::player::PlayerDto)) -> Vec<u8> {
    let mut session = common::load_fixture_session("v1_relics");
    let data = game_data();
    let player_id = first_player_id(&mut session);
    player::get_player_details(&mut session, &data, player_id, &null_progress())
