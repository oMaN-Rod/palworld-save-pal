...
use ps_core::domain::{guild, pal, player};
...
fn update_pals_edit_then_reread() {
    ...
    let collector: ps_core::progress::ProgressSink = std::sync::Arc::new(move |message: &str| {
...
fn update_pals_missing_pal_errors() {
    ...
    let edited = ps_core::dto::pal::PalDto {
        instance_id: ghost,
        ...
        is_tower: false,
        gender: ps_core::dto::pal::PalGender::Female,
        nickname: None,
...
fn update_players_common_container_edit_ignores_forged_dto_id() {
    ...
    common
        .slots
        .push(ps_core::dto::container::ItemContainerSlotDto {
            dynamic_item: None,
...
fn update_players_full_dto_survives_missing_quest_array_schema() {
...

    {
        ...
        let save_data_property = loaded
            ...
            .properties
            .0
            .get_mut(&ps_core::ue::PropertyKey::from("SaveData"))
            .expect("player has SaveData");
        let save_data =
            ps_core::props::struct_props_mut(save_data_property).expect("SaveData is a struct");
        save_data
            .0
            .shift_remove(&ps_core::ue::PropertyKey::from("CompletedQuestArray"));
        save_data
            .0
            .shift_remove(&ps_core::ue::PropertyKey::from("OrderedQuestArray"));

        let mut stripped_schemas = ps_core::ue::PropertySchemas::new();
    ...
    let reparsed = ps_core::savio::read_sav_bytes(sav_bytes).expect("reparse written .sav");
    let save_data_property =
        ps_core::props::get(&reparsed.root.properties, &["SaveData"]).expect("SaveData present");
    let save_data =
        ps_core::props::struct_props(save_data_property).expect("SaveData is a struct");

    let completed = ps_core::props::get(save_data, &["CompletedQuestArray"])
        .and_then(ps_core::props::name_values)
        .expect("CompletedQuestArray round trips as a Name array");
    assert_eq!(completed, &vec!["Main_UnlockFastTravel".to_string()]);

    let ordered = ps_core::props::get(save_data, &["OrderedQuestArray"])
        .and_then(ps_core::props::struct_values)
        .expect("OrderedQuestArray round trips as a Struct array");
    assert_eq!(ordered.len(), 1);
    let ps_core::ue::StructValue::Struct(quest) = &ordered[0] else {
        panic!("OrderedQuestArray element must be a Struct");
    };
    let quest_name = ps_core::props::get(quest, &["QuestName"])
...
fn typed_relic_write_creates_first_entry_in_an_empty_by_type_array() {
...

    {
        ...
        let record_data_property = save_data
            .0
            .get_mut(&ps_core::ue::PropertyKey::from("RecordData"))
            .expect("player has RecordData");
        let record_data = ps_core::props::struct_props_mut(record_data_property)
            .expect("RecordData is a struct");
        let by_type_property = record_data
            .0
            .get_mut(&ps_core::ue::PropertyKey::from(
                "RelicObtainForInstanceFlagByType",
            ))
            .expect("fixture sanity: player has RelicObtainForInstanceFlagByType");
        let by_type_values = ps_core::props::struct_values_mut(by_type_property)
...
fn v1_quest_session() -> (ps_core::session::SaveSession, GameData, Uuid) {
    let mut session = common::load_fixture_session("v1_relics");
    let data = game_data();
...
fn v1_save_mission_edit_persists() {
    ...
    assert!(
        !dto.completed_missions
            .contains(&"PS_TEST_QUEST".to_string()),
        "fixture sanity: the appended quest must not already be present"
    );
    dto.completed_missions.push("PS_TEST_QUEST".to_string());
    let mut modified = OrderedMap::new();
    modified.insert(player_id, dto);
    player::update_players(&mut session, &data, &modified, &null_progress()).unwrap();
