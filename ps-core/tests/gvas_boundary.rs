...
fn write_gvas_is_gvas_prefixed_and_round_trips() {
    ...
    let save = ps_core::savio::read_sav_bytes(&sav).expect("parse .sav");
    let gvas = ps_core::savio::write_gvas_bytes(&save).expect("serialize GVAS");
    assert_eq!(&gvas[0..4], b"GVAS", "write_gvas_bytes must emit raw GVAS");

    let reparsed = ps_core::savio::read_gvas_bytes(&gvas).expect("parse GVAS");
    let regvas = ps_core::savio::write_gvas_bytes(&reparsed).expect("re-serialize");
...
fn gvas_getters_round_trip_and_load_ingests_gvas() {
    let sav = world1_level_sav();
    let session = ps_core::session::SaveSession::load(
    ...
    ps_core::savio::read_gvas_bytes(&level_gvas).expect("level gvas re-parses");

    // SaveSession::load ingests that same raw GVAS (pass-through) into an equivalent session.
    let from_gvas = ps_core::session::SaveSession::load(
        ps_core::session::SaveKind::InMemory,
        "world1".to_string(),
        "steam",
        &level_gvas,
