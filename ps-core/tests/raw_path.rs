...
use ps_core::domain::raw_path::{NodeKind, RawPath, RawScalar, RawScope, VisitAction};
...
fn a_byte_level_field_round_trips_through_set_and_reparse() {
    ...
    let reparsed = ps_core::savio::read_sav_bytes(&bytes).expect("the edited level re-parses");
    let mut reparsed_session = ps_core::session::SaveSession::new_for_tests(
...
fn a_visit_can_remove_matching_nodes_and_they_stay_removed() {
    ...
    let count = |session: &mut ps_core::session::SaveSession| {
        session
...
fn a_host_driven_walk_removal_invalidates_caches() {
    ...
    let _ = ps_core::domain::guild::find_player_guild_id(&mut session, uid).expect("resolves");
