...
use ps_core::domain::guild_tail;
use ps_core::domain::{guild, player, world};
use ps_core::error::CoreError;
use ps_core::gamedata::GameData;
use ps_core::progress::null_progress;
use ps_core::props;
use ps_core::session::{LoadedPlayer, PlayerFileData, SaveKind, SaveSession};
use ps_core::ue::{
...
// world1 has 2 guilds, each with exactly one player (that guild's sole admin), one
// base whose worker container is empty but owns 4 storage containers, and one chest.
...
/// The check must not lazily load the guild as a side effect.
#[test]
...
/// `delete_guild_and_players` deliberately leaves the guild's own chest behind as an
/// orphan, for save-file fidelity with the game.
#[test]
...
// Both world1 guilds have exactly one player, so no fixture here can exercise
// deleting a NON-admin player; the synthetic two-player guild below is.
...
fn player_character_entry(player_id: Uuid) -> MapEntry {
    ...
    let character_data = ps_core::ue::games::palworld::PalCharacterData {
        object: object_props,
        unknown_bytes: [0; 4],
        group_id: ps_core::ue::FGuid::nil(),
        trailing_bytes: [0; 4],
    };
    let mut value_props = Properties::default();
    value_props.insert(
        "RawData",
        Property::Struct(StructValue::Game(ps_core::ue::PalStruct::CharacterData(character_data))),
    );
    MapEntry {
...
fn character_container_entry_with_pal(container_id: Uuid, slot_num: i32, pal_id: Uuid) -> MapEntry {
    ...
    slot_props.insert(
        "RawData",
        Property::Struct(StructValue::Game(ps_core::ue::PalStruct::CharacterContainer(
            ps_core::ue::games::palworld::PalCharacterContainer {
...
fn guild_group_entry(
    guild_id: Uuid,
    handle_ids: &[Uuid],
    guild: ps_core::ue::games::palworld::PalGuildGroup,
) -> MapEntry {
    ...
    let group_data = ps_core::ue::games::palworld::PalGroupData {
        group_id: props::uuid_to_guid(guild_id),
        group_name: String::new(),
        individual_character_handle_ids: handle_ids
            .iter()
            .map(|id| ps_core::ue::games::palworld::PalInstanceId {
                guid: props::uuid_to_guid(props::EMPTY_UUID),
                instance_id: props::uuid_to_guid(*id),
            })
            .collect(),
        data: ps_core::ue::games::palworld::PalGroupVariant::Guild(guild),
    };
    value_properties.insert(
        "RawData",
        Property::Struct(StructValue::Game(ps_core::ue::PalStruct::GroupData(group_data))),
    );
    MapEntry {
...
/// Both players' guild handles AND the pal's are recorded in
/// `individual_character_handle_ids` up front, so a delete's handle cleanup (or lack
/// of it) is directly observable afterward.
fn two_player_guild_session(guild_loaded: bool) -> TwoPlayerGuild {
    ...
    let member_pal_entry = ps_core::domain::pal::new_pal_entry(
        "SheepBall",
        member_pal_id,
        member_id,
        member_pal_box,
...
fn group_data_for(session: &SaveSession, guild_id: Uuid) -> &ps_core::ue::games::palworld::PalGroupData {
    let entries = world::group_map(&session.level).unwrap();
    let entry = entries
        .iter()
...
fn delete_non_admin_player_removes_everything_and_leaves_the_admin_byte_identical() {
    ...
    // `is_none()` alone would also hold for a mutation that never happened, so the
    // rebuilt index is checked too.
...
/// The whole-player delete path never calls `delete_character_handle` for box/party
/// pals, unlike a single-pal delete. Deliberate, for save-file fidelity with the game.
#[test]
...
/// The admin-refusal check is scoped to `session.loaded_guilds`, never the raw
/// `GroupSaveDataMap`: an admin whose guild was never loaded counts as guildless.
#[test]
...
// world1's base storage containers hold zero dynamic items, so the dynamic-item
// cascade has no real-save coverage; the synthetic container below is its only coverage.
...
fn item_container_entry_with_dynamic_slot(container_id: Uuid, local_id: Uuid) -> MapEntry {
    ...
    slot_props.insert(
        "RawData",
        Property::Struct(StructValue::Game(ps_core::ue::PalStruct::ItemContainerSlots(
            ps_core::ue::games::palworld::PalItemContainerSlot {
                slot_index: 0,
                count: 1,
                item: ps_core::ue::games::palworld::PalItemId {
                    static_id: "WeaponFire_Bow".to_string(),
                    dynamic_id: ps_core::ue::games::palworld::PalDynamicId {
                        created_world_id: ps_core::ue::FGuid::nil(),
...
fn dynamic_item_value(local_id: Uuid) -> StructValue {
    ...
    item_props.insert(
        "RawData",
        Property::Struct(StructValue::Game(ps_core::ue::PalStruct::DynamicItem(Box::new(
            ps_core::ue::games::palworld::PalDynamicItem {
                id: ps_core::ue::games::palworld::PalDynamicId {
                    created_world_id: ps_core::ue::FGuid::nil(),
                    local_id_in_created_world: props::uuid_to_guid(local_id),
                },
                static_id: "WeaponFire_Bow".to_string(),
                item_type: ps_core::ue::games::palworld::PalDynamicItemType::Unknown { trailer: vec![] },
            },
        )))),
    );
...
/// Deleting a container cascades to the `DynamicItemSaveData` entry its slot
/// referenced, resetting both caches.
#[test]
fn delete_item_containers_cascades_its_dynamic_item_and_invalidates_both_indexes() {
    ...
    ps_core::domain::containers::delete_item_containers(&mut session, &[container_id]).unwrap();
...
fn map_object_base_id(value: &StructValue) -> Option<Uuid> {
    ...
    match model.0.get(&PropertyKey::from("RawData")) {
        Some(Property::Struct(StructValue::Game(ps_core::ue::PalStruct::MapModel(raw)))) => {
...
/// Every `ItemContainer`/`CharacterContainer` module's `target_container_id`
/// on one `MapObjectSaveData` element.
fn map_object_container_target_ids(value: &StructValue) -> Vec<Uuid> {
    use ps_core::ue::games::palworld::PalMapConcreteModelModuleData;
    ...
    for module in modules {
        ...
        if let Some(Property::Struct(StructValue::Game(
            ps_core::ue::PalStruct::MapConcreteModelModule(raw),
        ))) = module_props.0.get(&PropertyKey::from("RawData"))
        {
            match &raw.data {
                PalMapConcreteModelModuleData::ItemContainer {
                    target_container_id,
                    ..
                } => ids.push(props::guid_to_uuid(target_container_id)),
...
fn work_base_id(value: &StructValue) -> Option<Uuid> {
    let StructValue::Struct(work_props) = value else {
        return None;
    };
    match work_props.0.get(&PropertyKey::from("RawData")) {
        Some(Property::Struct(StructValue::Game(ps_core::ue::PalStruct::Work(raw)))) => raw
            .base_data
            .as_ref()
            .map(|base_data| props::guid_to_uuid(&base_data.base_camp_id_belong_to)),
        _ => None,
    }
}

/// The final referential-integrity scan is the whole point: a dangling reference to a
/// removed id here is the exact crash-the-game bug class this feature exists to avoid.
#[test]
fn delete_base_removes_everything_of_one_base_and_leaves_no_dangling_reference() {
    ...
    let guild_before = guild::get_guild_details(&mut session, &data, guild_id)
        .unwrap()
        .expect("fixture guild loads");
    let base = guild_before
        .bases
        .as_ref()
        .and_then(|bases| bases.get(&base_id))
    ...
    assert!(
        guild_tail_before
            .base_ids
            .iter()
            .any(|id| props::guid_to_uuid(id) == base_id),
        "test precondition: the guild tail must list the target base before delete"
    );
    let other_base_ids: Vec<Uuid> = guild_tail_before
        .base_ids
        .iter()
        ...
        .collect();
    assert!(
        !other_base_ids.is_empty(),
        "fixture guild must own another base too, or the guild-survives assertions below \
         are vacuous"
    );
    ...
    assert!(
        world::base_camp_map(&session.level)
            ...
            .iter()
            .all(|e| props::as_uuid(&e.key) != Some(base_id)),
        "the base camp entry must be removed"
    );
    ...
    let works_after = world::work_values(&session.level).unwrap().unwrap();
    assert!(
        works_after.iter().all(|v| work_base_id(v) != Some(base_id)),
        "no WorkSaveData element may still reference the deleted base"
    );
    assert_eq!(
        works_after.len(),
        works_before_len - matching_works_before,
        "only the deleted base's own work entries may be removed"
    );
    let other_works_after: Vec<StructValue> = works_after
        .iter()
        .filter(|v| work_base_id(v) != Some(base_id))
        .cloned()
        .collect();
    assert_eq!(
        other_works_before, other_works_after,
        "every other base's work entries must survive byte-for-byte"
    );
    ...
    let character_containers_after = world::character_container_map(&session.level).unwrap();
    assert!(
        !character_containers_after
            .iter()
            .any(|e| container_id_key(e) == Some(worker_container_id)),
        "the base's worker container must be deleted along with its base"
    );
    ...
    let guild_tail_after = {
        ...
        let entry = entries
            .iter()
            .find(|e| props::as_uuid(&e.key) == Some(guild_id))
            .expect("the guild's own group entry must still exist");
        let group_data = guild_tail::entry_group_data(entry).unwrap();
        guild_tail::as_guild(group_data).unwrap().clone()
    };
    ...
    let mut removed_ids: std::collections::HashSet<Uuid> = item_container_ids.into_iter().collect();
    removed_ids.insert(worker_container_id);
    for map_object in map_objects_after {
        for target_id in map_object_container_target_ids(map_object) {
            assert!(
                !removed_ids.contains(&target_id),
                "a surviving map object must not reference a deleted container {target_id}"
            );
        }
    }

    let bytes = session.level_sav_bytes().expect("level must serialize after delete_base");
    ps_core::savio::read_sav_bytes(&bytes).expect("level written after delete_base must parse back");
}

#[test]
fn delete_base_with_an_unknown_id_is_an_error_and_mutates_nothing() {
    let mut session = common::load_corpus_session();
    let data = game_data();
    ...
    assert!(matches!(error, CoreError::Other(message) if message.contains("not found")));
    assert_eq!(
        world::base_camp_map(&session.level).unwrap().unwrap().len(),
        base_camps_before
    );
    assert_eq!(
