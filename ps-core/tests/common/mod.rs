use ps_core::domain::blueprint::validate::Anchor;
...
pub fn all_player_uids(session: &SaveSession) -> Vec<Uuid> {
    ...
    if let Ok(entries) = ps_core::domain::world::character_map(&session.level) {
        uids.extend(entries.iter().filter_map(ps_core::domain::world::entry_player_uid));
    }
...
pub fn all_group_ids(session: &SaveSession) -> Vec<Uuid> {
    let mut ids: BTreeSet<Uuid> = ps_core::domain::world::group_map(&session.level)
        .map(|entries| entries.iter().filter_map(|entry| ps_core::props::as_uuid(&entry.key)).collect())
        .unwrap_or_default();
    ids.remove(&Uuid::nil());
...
pub fn player_sav_json(session: &SaveSession, player_id: Uuid) -> serde_json::Value {
    ...
    let save = ps_core::savio::read_sav_bytes(sav_bytes).expect("parse player sav");
...
pub fn first_player_with_relics(session: &mut SaveSession, data: &GameData) -> Uuid {
    ...
    for id in ids {
        if ps_core::domain::player::get_player_details(session, data, id, &null_progress())
            .ok()
            .flatten()
            .is_none()
        {
...
#[allow(dead_code)]
pub fn container_slot_census(entry: &ps_core::ue::MapEntry) -> (usize, usize) {
    use ps_core::ue::{PalStruct, Property, PropertyKey, StructValue};

    let Some(value_props) = ps_core::props::struct_props(&entry.value) else { return (0, 0) };
    let Some(slots) =
    ...
    for slot in slots {
        ...
        {
            total += 1;
            if !ps_core::props::guid_to_uuid(&raw.instance_id).is_nil() {
                occupied += 1;
            }
        }
    }
...
pub fn fixture_base_id(session: &SaveSession) -> Uuid {
    ...
    let map_objects = ps_core::domain::world::map_object_values(&session.level)
    ...
    for value in map_objects {
        ...
        let base_id = ps_core::props::guid_to_uuid(&raw.base_camp_id_belong_to);
        if base_id.is_nil() {
            continue;
        }
    ...
    let base_camps = ps_core::domain::world::base_camp_map(&session.level)
...
pub fn fixture_guild_id(session: &SaveSession) -> Uuid {
    ...
    ps_core::props::guid_to_uuid(&fixture_base_camp_raw(session, base_id).group_id_belong_to)
}
...
pub fn base_camp_guild_ids(session: &SaveSession) -> Vec<Uuid> {
    ...
    ps_core::domain::world::base_camp_map(&session.level)
        ...
        .iter()
        .filter_map(|entry| {
            let value_props = ps_core::props::struct_props(&entry.value)?;
            match value_props.0.get(&PropertyKey::from("RawData")) {
                Some(Property::Struct(StructValue::Game(PalStruct::BaseCamp(raw)))) => {
                    Some(ps_core::props::guid_to_uuid(&raw.group_id_belong_to))
                }
                _ => None,
            }
        })
...
pub fn set_base_area_range(session: &mut SaveSession, base_id: Uuid, area_range: f32) {
    ...
    let entry = entries
        ...
        .find(|entry| ps_core::props::as_uuid(&entry.key) == Some(base_id))
        .expect("fixture base camp entry exists");
    let value_props = ps_core::props::struct_props_mut(&mut entry.value).expect("base camp value");
...
#[allow(dead_code)]
pub fn load_world_option_fixture(name: &str) -> ps_core::ue::Save {
    ...
    let mut save = ps_core::savio::read_sav_bytes(&bytes).expect("parse WorldOption fixture");
    ps_core::domain::world_option::ensure_world_option_schemas(&mut save);
    save
}
...
pub fn set_world_option_int(session: &mut SaveSession, key: &str, value: i32) {
    use ps_core::domain::world_option;
...
pub fn fixture_player_uid(session: &SaveSession) -> Uuid {
    ...
    let entries = ps_core::domain::world::group_map(&session.level).expect("group map");
    let entry = entries
        .iter()
        .find(|entry| ps_core::props::as_uuid(&entry.key) == Some(guild_id))
        .expect("the fixture guild has a GroupSaveDataMap entry");
    let group_data =
        ps_core::domain::guild_tail::entry_group_data(entry).expect("guild group data");
    let guild = ps_core::domain::guild_tail::as_guild(group_data).expect("group is a guild");
    let players = ps_core::domain::guild_tail::guild_player_uids(guild);
...
pub fn world_option_int(session: &SaveSession, key: &str) -> Option<i64> {
    ps_core::domain::world_option::read_settings(session.world_option.as_ref()?)
        .into_iter()
