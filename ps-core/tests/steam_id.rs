...
fn conversions_match_python_reference_vectors() {
    ...
    for vector in vectors {
        let uid = ps_core::steam_id::steam_id_to_player_uid(vector.steam_id);
        assert_eq!(
            uid.to_string(),
            vector.palworld_uid,
            "steam_id {}",
        ...
        assert_eq!(
            ps_core::steam_id::player_uid_to_nosteam(uid),
            vector.nosteam_uid
        );
    }
}
...
fn input_parsing_accepts_all_supported_formats() {
    assert_eq!(
        ps_core::steam_id::parse_steam_input(
    ...
    assert_eq!(
        ps_core::steam_id::parse_steam_input("steam_42").unwrap(),
        42
    );
    assert!(matches!(
    ...
    assert!(!ps_core::steam_id::is_palworld_uid("not-a-uid"));
    assert_eq!(
        ps_core::steam_id::parse_palworld_uid("AABBCCDD000000000000000000000000")
            .unwrap()
            .to_string(),
        "aabbccdd-0000-0000-0000-000000000000"
    );
