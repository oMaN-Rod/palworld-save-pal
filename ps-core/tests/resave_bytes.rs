...
use ps_core::domain::{guild_tail, pal, player};
...
fn edited_pal_reloads_with_edit_applied_and_entry_count_stable() {
    ...
    let entry_count = ps_core::domain::world::character_map(&session.level)
        .unwrap()
        .len();
    let sav_bytes = session.level_sav_bytes().expect("write level sav");
    let reloaded = ps_core::savio::read_sav_bytes(&sav_bytes).expect("re-read level sav");

    assert_eq!(
        ps_core::domain::world::character_map(&reloaded)
            .unwrap()
            .len(),
        entry_count,
        "editing a pal must not change the entry count"
    );
    let reloaded_entry = ps_core::domain::world::character_map(&reloaded)
        .unwrap()
        .iter()
...
fn edit_one_pal_leaves_guild_tails_byte_identical() {
    ...
    let untouched_tails: Vec<(uuid::Uuid, ps_core::ue::games::palworld::PalGroupVariant)> =
        ps_core::domain::world::group_map(&session.level)
            .unwrap()
            .iter()
            .filter_map(|entry| {
                let guild_id = ps_core::props::as_uuid(&entry.key)?;
    ...
    for entry in ps_core::domain::world::group_map(&session.level).unwrap() {
        let Some(guild_id) = ps_core::props::as_uuid(&entry.key) else {
            continue;
        };
...
fn rename_world_updates_meta_and_survives_resave() {
    ...
    let meta_bytes = session
        ...
        .expect("write meta")
        .expect("meta present");
    let reloaded_meta = ps_core::savio::read_sav_bytes(&meta_bytes).expect("re-read meta");
    let world_name =
        ps_core::props::get(&reloaded_meta.root.properties, &["SaveData", "WorldName"])
...
fn edited_player_save_out_succeeds_after_boss_technology_point_fix() {
    ...
    ps_core::savio::read_sav_bytes(sav_bytes).expect("edited player .sav re-reads cleanly");
}
...
fn edited_player_level_sav_bytes_succeeds_and_edit_round_trips() {
    ...
    let reloaded = SaveSession::new_for_tests(
        SaveKind::InMemory,
        ps_core::savio::read_sav_bytes(&level_bytes).expect("edited Level.sav re-reads cleanly"),
    );
    let entries = ps_core::domain::world::character_map(&reloaded.level).unwrap();
    let reloaded_entry = entries
        .iter()
        .find(|entry| {
            ps_core::domain::world::entry_is_player(entry)
                && ps_core::domain::world::entry_player_uid(entry) == Some(player_id)
        })
        .expect("edited player still present in the reloaded Level.sav");
    let save_parameter = ps_core::domain::world::entry_save_parameter(reloaded_entry)
        .expect("reloaded player entry has a save parameter");
    let level_after = save_parameter
        .0
        .get(&ps_core::ue::PropertyKey::from("Level"))
        .expect("Level property present");
    assert_eq!(
        level_after,
        &ps_core::props::byte_property(60),
...
fn add_player_pal_then_resave_succeeds_and_pal_round_trips() {
    ...
    let reloaded = ps_core::savio::read_sav_bytes(&sav_bytes).expect("re-read level sav");
    let survived = ps_core::domain::world::character_map(&reloaded)
        .unwrap()
        .iter()
        .any(|entry| {
            ps_core::domain::world::entry_instance_id(entry) == Some(new_pal.instance_id)
        });
    assert!(
        survived,
...
fn read_level_only() -> ps_core::ue::Save {
    let level_bytes = fixture_file("world1/Level.sav");
    ps_core::savio::read_sav_bytes(&level_bytes).expect("parse level")
}
