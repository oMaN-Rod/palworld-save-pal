...
use ps_core::domain::blueprint::{capture, gvas, remap, BaseBlueprint, CaptureOptions};
...
fn definition_sets(blueprint: &BaseBlueprint) -> Vec<(&'static str, HashSet<Uuid>)> {
    vec![
        ...
        ("structure concrete models", set_of(capture::structure_concrete_instance_ids(blueprint))),
        ("works", set_of(blueprint.works.iter().filter_map(capture::work_base_id))),
        ("work assigns", set_of(blueprint.works.iter().flat_map(work_assign_ids))),
        ("item containers", set_of(blueprint.item_containers.iter().filter_map(capture::container_entry_id))),
        (
            "character containers",
            set_of(blueprint.character_containers.iter().filter_map(capture::container_entry_id)),
        ),
        ("characters", set_of(blueprint.characters.iter().filter_map(world::entry_instance_id))),
        ("dynamic items", set_of(blueprint.dynamic_items.iter().filter_map(capture::dynamic_item_local_id))),
    ]
}
...
fn connector(properties: &Properties) -> Option<&PalConnector> {
    ...
    let connector = model.0.get(&PropertyKey::from("Connector")).and_then(props::struct_props)?;
...
fn work_assign_individual_ids(work: &StructValue) -> Vec<Uuid> {
    work_assigns(work).into_iter().map(|(_, individual)| individual).collect()
}
...
fn work_assigns(work: &StructValue) -> Vec<(Uuid, Uuid)> {
    ...
    for entry in entries {
        let Some(assign_props) = props::struct_props(&entry.value) else { continue };
...
fn work_owner_ids(work: &StructValue) -> Vec<(&'static str, Uuid)> {
    ...
    if let Some(base) = &raw.base_data {
        out.push(("owner_map_object_model_id", props::guid_to_uuid(&base.owner_map_object_model_id)));
        out.push((
            "owner_map_object_concrete_model_id",
            props::guid_to_uuid(&base.owner_map_object_concrete_model_id),
        ));
    }
    if let Some(id) = raw.transform.as_ref().and_then(|t| t.map_object_instance_id.as_ref()) {
...
fn module_targets(properties: &Properties) -> Vec<(&'static str, Uuid)> {
    ...
    let Some(entries) = concrete.0.get(&PropertyKey::from("ModuleMap")).and_then(props::map_entries)
    else {
        return out;
    };
    for module in entries {
        ...
        match &raw.data {
            ...
            PalMapConcreteModelModuleData::CharacterContainer { target_container_id, .. } => {
                out.push(("character containers", props::guid_to_uuid(target_container_id)));
            }
    ...
    container_slot_ids(entry, |slot_props| {
        match slot_props.0.get(&PropertyKey::from("RawData")) {
            Some(Property::Struct(StructValue::Game(PalStruct::ItemContainerSlots(raw)))) => {
                Some(props::guid_to_uuid(&raw.item.dynamic_id.local_id_in_created_world))
            }
            _ => None,
        }
    })
...
fn container_slot_ids(
    ...
    let mut out = Vec::new();
    let Some(value_props) = props::struct_props(&entry.value) else { return out };
...
fn base_camp_owner_id(blueprint: &BaseBlueprint) -> Option<Uuid> {
    match blueprint.base_camp.as_ref()?.0.get(&PropertyKey::from("RawData"))? {
...
fn remapping_gives_every_structure_a_fresh_instance_id() {
    ...
    let before: HashSet<Uuid> = capture::structure_instance_ids(&blueprint).into_iter().collect();
    remap::remap_blueprint(&mut blueprint).expect("remap");
    let after: HashSet<Uuid> = capture::structure_instance_ids(&blueprint).into_iter().collect();

    assert!(!before.is_empty(), "the fixture base must have structures for this to mean anything");
...
fn remapping_twice_yields_two_disjoint_id_sets() {
    ...
    let a: HashSet<Uuid> = capture::structure_instance_ids(&first).into_iter().collect();
    let b: HashSet<Uuid> = capture::structure_instance_ids(&second).into_iter().collect();
    assert!(!a.is_empty(), "the fixture base must have structures for this to mean anything");
    assert!(a.is_disjoint(&b), "the same blueprint placed twice must not collide with itself");
}
...
fn every_definition_set_is_non_empty_and_fully_regenerated() {
    ...
    for ((name, before), (_, after)) in before.into_iter().zip(after) {
        assert!(!before.is_empty(), "{name}: the fixture must define ids here, or this is vacuous");
        assert_eq!(before.len(), after.len(), "{name}: no definition may be lost or merged");
        assert!(before.is_disjoint(&after), "{name}: every defined id must be regenerated");
    }
}
...
fn every_reference_resolves_into_the_post_remap_definition_set() {
    ...
    let mut check = |label: &str, id: Uuid, targets: &HashSet<Uuid>| {
        ...
        assert!(targets.contains(&id), "{label}: {id} resolves to nothing in the blueprint");
        checked += 1;
    };
...
fn lookup<'a>(sets: &'a [(&'static str, HashSet<Uuid>)], name: &str) -> &'a HashSet<Uuid> {
    &sets.iter().find(|(key, _)| *key == name).expect("known definition set").1
}
...
fn character_container_slots_still_name_their_pals() {
    ...
    let characters = set_of(blueprint.characters.iter().filter_map(world::entry_instance_id));
    let slots = set_of(blueprint.character_containers.iter().flat_map(character_container_slot_ids));

    assert!(!characters.is_empty(), "the fixture base must have housed or worker pals");
    assert_eq!(
        slots.len(),
        characters.len(),
        "every occupied slot must still name exactly one captured pal"
    );
    assert!(slots.is_subset(&characters), "a slot must not point at a pal outside the blueprint");
}
...
fn work_assignments_still_name_their_workers() {
    ...
    assert!(!assigned_before.is_empty(), "the fixture must have assigned workers");
    assert_eq!(assigned_after.len(), assigned_before.len(), "no assignment may be dropped to nil");
    assert!(assigned_after.is_subset(&characters), "an assignment must name a captured pal");
}
...
fn the_work_collection_lists_exactly_the_remapped_works() {
    ...
    let before_works: Vec<Uuid> = blueprint.works.iter().filter_map(capture::work_base_id).collect();
    remap::remap_blueprint(&mut blueprint).expect("remap");
    let after = work_collection(&blueprint);
    let after_works: Vec<Uuid> = blueprint.works.iter().filter_map(capture::work_base_id).collect();
...
/// survive the encoders that turn a blueprint into a `.psbp` file.
#[test]
fn a_remapped_blueprint_still_round_trips_through_both_encodings() {
    ...
    let from_psbp = gvas::from_psbp_bytes(&gvas::to_psbp_bytes(&blueprint).expect("psbp encode"))
        .expect("psbp decode");
    let from_json =
        gvas::from_json(&gvas::to_json(&blueprint).expect("json encode")).expect("json decode");

    assert_eq!(from_psbp.structures.len(), blueprint.structures.len());
    assert_eq!(work_collection(&from_json), expected, "the rebuilt WorkCollection must survive json");
    assert_eq!(
        connector_tail_hits(&from_json, &set_of(capture::structure_instance_ids(&from_json))),
        connector_tail_hits(&blueprint, &set_of(capture::structure_instance_ids(&blueprint))),
...
fn a_nil_id_stays_nil() {
    ...
    assert!(remap.new_for(Uuid::nil()).is_nil(), "a zero guid must not be given an identity");
}
...
fn every_uid_needle_form_is_a_distinct_findable_needle() {
    ...
    assert_eq!(forms.len(), 3, "every encoding a uid can appear in must be scanned for");

    for (form, needle) in &forms {
        assert!(!needle.is_empty(), "{form}: an empty needle cannot be searched for");
        let mut haystack = vec![0xABu8; 7];
        haystack.extend_from_slice(needle);
        haystack.extend_from_slice(&[0xCDu8; 7]);
        assert!(contains(&haystack, needle), "{form}: a planted needle must be found");
...
/// A `.psbp` file's payload, decompressed. The file is Oodle-compressed, so a byte
/// scan over it as written would pass with every UID still inside.
fn ps_payload(blueprint: &BaseBlueprint) -> Vec<u8> {
    let file = gvas::to_psbp_bytes(blueprint).expect("psbp encode");
    let body = &file[gvas::PSBP_MAGIC.len() + 4..];
    let payload =
        ps_core::ue::compression::decompress_save(&mut std::io::Cursor::new(body))
            .expect("a psbp body must decompress");
    assert!(
        payload.len() > file.len(),
        "setup: the psbp body must actually be compressed, {} raw vs {} in the file",
        payload.len(),
        file.len()
    );
    payload
...
#[test]
fn no_player_uid_survives_into_the_ps_bytes() {
    let session = common::load_fixture_session("v1_relics");
    let base_id = common::fixture_base_id(&session);
    let uids = common::all_player_uids(&session);
    ...
    for (layer, options) in capture_layers() {
        let leaky = capture::capture_unscrubbed(&session, base_id, options, "Home")
            .expect("unscrubbed capture");
        let leaky_payload = ps_payload(&leaky);
        ...
        let payload = ps_payload(&blueprint);
        for uid in &uids {
            for (form, needle) in uid_needles(*uid) {
                assert!(
                    !contains(&payload, &needle),
                    "{layer}: player uid {uid} leaked into the psbp bytes as {form}"
                );
            }
        }
    }
...
fn no_guild_id_from_the_source_save_survives_capture() {
    ...
    for (layer, options) in capture_layers() {
        let leaky = capture::capture_unscrubbed(&session, base_id, options, "Home")
            .expect("unscrubbed capture");
        assert!(
            gvas::to_json(&leaky).expect("serialize").contains(&owning_guild.to_string()),
            "{layer}: an unscrubbed capture must carry the source guild id, \
             or the json scan proves nothing"
        );
        assert!(
            contains(&ps_payload(&leaky), &palbin::guid_bytes(owning_guild)),
            "{layer}: an unscrubbed capture must carry the source guild id, \
        ...
        for id in &group_ids {
            ...
            for (form, needle) in uid_needles(*id) {
                assert!(
                    !contains(&payload, &needle),
                    "{layer}: guild id {id} leaked into the psbp bytes as {form}"
                );
            }
        }
    }
...
fn no_captured_pal_still_names_its_guild() {
    ...
    let blueprint = capture::capture(&session, base_id, CaptureOptions::full(), "Home")
        .expect("capture");
...
const INJECTED_PASSWORD: &str = "PsBlueprintLockSecret";
...
fn inject_password_locks(
    ...
    use ps_core::ue::games::palworld::{PalMapConcreteModelModule, PalPlayerLockInfo};
...
fn structure_locks(properties: &Properties) -> Vec<(String, Vec<Uuid>)> {
    ...
    for module in entries {
            ...
            &raw.data
        {
            out.push((
                password.clone(),
                player_infos.iter().map(|info| props::guid_to_uuid(&info.player_uid)).collect(),
            ));
        }
    }
    out
...
fn a_lock_password_leaves_the_save_only_in_a_full_capture() {
    ...
    for (layer, options) in capture_layers() {
        ...
        assert_eq!(
            lock_player_uids(&leaky).iter().filter(|uid| **uid == victim).count(),
            control,
            "{layer}: an unscrubbed capture must carry the locked-out player exactly when \
             the layer keeps the lock"
        );
        assert_eq!(
            contains(&ps_payload(&leaky), INJECTED_PASSWORD.as_bytes()),
        ...
        assert_eq!(
            contains(&ps_payload(&blueprint), INJECTED_PASSWORD.as_bytes()),
            keeps_password,
        ...
        if options.access_config {
            ...
            assert!(uids.len() >= LOCKS, "{layer}: setup: the injected locks must be in this list");
        }
        assert!(
...
fn destination_id_sets(session: &SaveSession) -> Vec<(&'static str, Vec<Uuid>)> {
    vec![
        ...
        ("concrete models", destination_concrete_instance_ids(session)),
        (
            "works",
        ...
        (
            ...
            world::item_container_map(&session.level)
                .map(|entries| entries.iter().filter_map(capture::container_entry_id).collect())
                .unwrap_or_default(),
        ),
        (
            "character containers",
        ...
        (
            ...
            world::character_map(&session.level)
                .map(|entries| entries.iter().filter_map(world::entry_instance_id).collect())
                .unwrap_or_default(),
        ),
        (
            "dynamic items",
            world::dynamic_item_values(&session.level)
                .map(|values| values.iter().filter_map(capture::dynamic_item_local_id).collect())
                .unwrap_or_default(),
        ),
    ]
}
...
fn blueprint_definition_counts(blueprint: &BaseBlueprint) -> Vec<(&'static str, usize)> {
    vec![
        ...
        ("works", blueprint.works.iter().filter_map(capture::work_base_id).count()),
        (
            "item containers",
            blueprint.item_containers.iter().filter_map(capture::container_entry_id).count(),
        ),
        (
            "character containers",
            blueprint.character_containers.iter().filter_map(capture::container_entry_id).count(),
        ),
        (
            "characters",
            blueprint.characters.iter().filter_map(world::entry_instance_id).count(),
        ),
        (
            "dynamic items",
            blueprint.dynamic_items.iter().filter_map(capture::dynamic_item_local_id).count(),
        ),
    ]
}
...
fn placement_introduces_no_guid_that_already_existed() {
    ...
    let before: HashSet<Uuid> = common::all_map_object_instance_ids(&session).into_iter().collect();
    assert!(
    ...
    let placed = ps_core::domain::guild::base_structures(&session, new_base_id);
    assert_eq!(
        placed.len(),
        FIXTURE_BASE_STRUCTURES,
    ...
    for structure in &placed {
        let id: Uuid = structure.instance_id.parse().expect("instance id is a uuid");
    ...
    {
        assert!(added > 0, "{name}: the blueprint must define ids here, or this is vacuous");
...
fn every_reference_a_placed_base_makes_resolves_in_the_destination() {
    ...
    let mut check = |label: &str, id: Uuid, targets: &HashSet<Uuid>| {
        ...
        assert!(targets.contains(&id), "{label}: {id} resolves to nothing in the destination");
        checked += 1;
    };
