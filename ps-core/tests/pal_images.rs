...
//!
//! This is NOT a crash guard. `ps-ui/src/lib/utils/assetLoader.ts` (`AssetLoader`)
...
//!
//! Filename derivation mirrors `ps-ui/src/lib/utils/assetLoader.ts`:
...
/// Mirrors `AssetLoader.cleanseCharacterId` in `ps-ui/src/lib/utils/assetLoader.ts`.
...
fn existing_assets() -> HashSet<String> {
    let img_dir = Path::new(env!("CARGO_MANIFEST_DIR")).join("../ps-ui/src/lib/assets/img");
...
fn every_pal_key_has_an_image_asset() {
    ...
    assert!(
        ...
        "{} pal(s) have no .webp asset and are not in KNOWN_MISSING_ART: {:?}\n\
         Either add the missing asset(s) under ps-ui/src/lib/assets/img, or if the \
         pal is a genuinely un-ownable internal entity (like a raid-boss body \
         part), deliberately add it to KNOWN_MISSING_ART in ps-core/tests/pal_images.rs \
    ...
    assert!(
        ...
        "these KNOWN_MISSING_ART entries now have art and should be removed \
         from the allow-list in ps-core/tests/pal_images.rs: {stale:?}"
    );
}
