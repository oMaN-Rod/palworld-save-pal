...
use ps_core::domain::blueprint::place::PlacementRequest;
use ps_core::domain::blueprint::validate::{Anchor, Finding, PlacementMode, Severity};
use ps_core::domain::blueprint::{capture, place, validate, BaseBlueprint, CaptureOptions};
...
fn horizontal_offset(structure: &ps_core::domain::blueprint::BlueprintStructure) -> f64 {
...
fn guild_registration_counts(entry: &ps_core::ue::MapEntry) -> (usize, usize, usize) {
...
fn non_guild_group_ids(session: &SaveSession) -> Vec<Uuid> {
    use ps_core::domain::{guild_tail, world};
    ...
    entries
        ...
        .filter_map(|entry| ps_core::props::as_uuid(&entry.key))
        .collect()
}
...
fn group_gate_facts(session: &SaveSession, group_id: Uuid) -> (Option<String>, bool, bool) {
    ...
    let entry = world::group_map(&session.level)
        .expect("group map")
        .iter()
        .find(|entry| ps_core::props::as_uuid(&entry.key) == Some(group_id))
...
fn relabel_group_as_guild(session: &mut SaveSession, group_id: Uuid) {
    ...
    let entry = world::group_map_mut(&mut session.level)
        ...
        .find(|entry| ps_core::props::as_uuid(&entry.key) == Some(group_id))
        .expect("the named group has a GroupSaveDataMap entry");
    ps_core::props::struct_props_mut(&mut entry.value)
        .expect("group value")
        .insert("GroupType", ps_core::props::enum_property("EPalGroupType::Guild"));
}
...
fn placed_structures(session: &SaveSession, base_id: Uuid) -> Vec<PlacedStructure> {
    use ps_core::ue::games::palworld::PalMapConcreteModelVariant;
    use ps_core::ue::{PalStruct, Property, PropertyKey, StructValue};

    let values = ps_core::domain::world::map_object_values(&session.level)
    ...
    values
        ...
        .filter_map(|value| {
            ...
            if ps_core::props::guid_to_uuid(&raw.base_camp_id_belong_to) != base_id {
                return None;
            }
                    ...
                    Property::Struct(StructValue::Game(PalStruct::MapConcreteModel(concrete))) => {
                        match &concrete.model_data {
                            PalMapConcreteModelVariant::BaseCampPoint(point) => {
                                Some(ps_core::props::guid_to_uuid(&point.base_camp_id))
                            }
                            _ => None,
                        }
                    }
            ...
            Some(PlacedStructure {
                map_object_id: object_props
                    ...
                    .and_then(ps_core::props::as_str)
                    .unwrap_or_default()
                    .to_string(),
                instance_id: ps_core::props::guid_to_uuid(&raw.instance_id),
                base_camp_id: ps_core::props::guid_to_uuid(&raw.base_camp_id_belong_to),
                group_id: ps_core::props::guid_to_uuid(&raw.group_id_belong_to),
                build_player_uid: ps_core::props::guid_to_uuid(&raw.build_player_uid),
                base_camp_point_id,
                x: translation.x.0,
                y: translation.y.0,
                z: translation.z.0,
...
fn a_placement_that_fails_after_validation_leaves_the_session_untouched() {
    ...
    untyped_base_camp
        ...
        .insert("RawData", ps_core::props::int_property(0));
    ...
    clashing_types.structures[0].properties.insert("PsTypeClash", ps_core::props::int_property(0));
    clashing_types.structures[1]
        .properties
        .insert("PsTypeClash", ps_core::ue::Property::Str("clash".to_string()));
...
fn a_placed_blueprint_still_serializes_and_parses_back() {
    ...
    let reparsed = ps_core::savio::read_sav_bytes(&bytes)
        .expect("level written after placement must parse back");
    let reloaded = ps_core::session::SaveSession::new_for_tests(
        ps_core::session::SaveKind::InMemory,
        reparsed,
    );
    assert_eq!(
...
fn first_guild_id(session: &SaveSession) -> Uuid {
    ...
    world::group_map(&session.level)
        ...
        .and_then(|entry| ps_core::props::as_uuid(&entry.key))
...
fn guild_member_uid(session: &SaveSession, guild_id: Uuid) -> Uuid {
    ...
    let entry = world::group_map(&session.level)
        .expect("group map")
        .iter()
        .find(|entry| ps_core::props::as_uuid(&entry.key) == Some(guild_id))
...
fn base_camp_entry(session: &SaveSession, base_id: Uuid) -> &ps_core::ue::MapEntry {
    ps_core::domain::world::base_camp_map(&session.level)
...
fn base_camp_raw(
    session: &SaveSession,
    base_id: Uuid,
) -> ps_core::ue::games::palworld::PalBaseCamp {
    use ps_core::ue::{PalStruct, Property, PropertyKey, StructValue};
...
fn guild_lists(session: &SaveSession, guild_id: Uuid) -> (Vec<Uuid>, Vec<Uuid>, Vec<Uuid>) {
    ...
    (
        group_data
            ...
            .iter()
            .map(|handle| ps_core::props::guid_to_uuid(&handle.instance_id))
            .collect(),
        guild.base_ids.iter().map(ps_core::props::guid_to_uuid).collect(),
        guild
...
fn work_bindings(session: &SaveSession) -> Vec<(Uuid, Uuid)> {
    ...
    ps_core::domain::world::work_values(&session.level)
        ...
        .filter_map(|value| {
            ...
            Some((
                ps_core::props::guid_to_uuid(&base.id),
                ps_core::props::guid_to_uuid(&base.base_camp_id_belong_to),
            ))
        })
        .collect()
}
...
fn character_container_slot_owners(session: &SaveSession) -> Vec<(Uuid, Uuid)> {
    ...
    for entry in
        ps_core::domain::world::character_container_map(&session.level).expect("containers")
    {
        let Some(container_id) = capture::container_entry_id(entry) else { continue };
        let Some(value_props) = ps_core::props::struct_props(&entry.value) else { continue };
        let Some(slots) =
            ps_core::props::get(value_props, &["Slots"]).and_then(ps_core::props::struct_values)
        else {
            continue;
        };
        for slot in slots {
            ...
            {
                owners.push((container_id, ps_core::props::guid_to_uuid(&raw.player_uid)));
            }
        }
    }
    owners
...
fn character_container_module_ids(properties: &ps_core::ue::Properties) -> Vec<Uuid> {
    ...
    let Some(modules) =
        concrete.0.get(&PropertyKey::from("ModuleMap")).and_then(ps_core::props::map_entries)
    else {
        return ids;
    };
    for module in modules {
        let Some(module_props) = ps_core::props::struct_props(&module.value) else { continue };
            ...
            module_props.0.get(&PropertyKey::from("RawData"))
        {
            if let PalMapConcreteModelModuleData::CharacterContainer { target_container_id, .. } =
                &raw.data
            {
                ids.push(ps_core::props::guid_to_uuid(target_container_id));
            }
        }
    }
    ids
...
fn character_container_entry(session: &SaveSession, container_id: Uuid) -> &ps_core::ue::MapEntry {
    ps_core::domain::world::character_container_map(&session.level)
        .expect("containers")
        .iter()
...
fn work_individual_refs(session: &SaveSession) -> Vec<(Uuid, Uuid)> {
    use ps_core::ue::games::palworld::PalWorkTypeSpecificData;
    use ps_core::ue::{PalStruct, Property, PropertyKey, StructValue};

    let mut refs = Vec::new();
    let Some(values) = ps_core::domain::world::work_values(&session.level).expect("work values")
    else {
        return refs;
    };
    for value in values {
        ...
        let work_id =
            raw.base_data.as_ref().map(|base| ps_core::props::guid_to_uuid(&base.id)).unwrap_or_default();
        match &raw.work_specific_data {
            PalWorkTypeSpecificData::Assign { assigned_individual_id, .. } => {
                refs.push((work_id, ps_core::props::guid_to_uuid(&assigned_individual_id.instance_id)));
            }
            PalWorkTypeSpecificData::ReviveCharacter { target_individual_id } => {
                refs.push((work_id, ps_core::props::guid_to_uuid(&target_individual_id.instance_id)));
            }
            _ => {}
        }
        if let Some(Property::Map(entries)) = work_props.0.get(&PropertyKey::from("WorkAssignMap")) {
            for entry in entries {
                let Some(assign_props) = ps_core::props::struct_props(&entry.value) else {
                    continue;
                };
                    ...
                    assign_props.0.get(&PropertyKey::from("RawData"))
                {
                    refs.push((
                        work_id,
                        ps_core::props::guid_to_uuid(&assign.assigned_individual_id.instance_id),
                    ));
                }
            }
        }
...
fn character_instance_ids(session: &SaveSession) -> BTreeSet<Uuid> {
    ps_core::domain::world::character_map(&session.level)
        .expect("characters")
        .iter()
        .filter_map(ps_core::domain::world::entry_instance_id)
        .collect()
}
...
fn a_blueprint_placed_into_another_save_leaves_that_save_writable() {
    ...
    for (layer, options) in [
        ...
        for target_name in ["v1_stats", "world1"] {
            ...
            let reparsed = ps_core::savio::read_sav_bytes(&bytes).unwrap_or_else(|error| {
                panic!("{layer} -> {target_name}: the written level must parse back: {error}")
            });
            let reloaded =
                SaveSession::new_for_tests(ps_core::session::SaveKind::InMemory, reparsed);
            assert_eq!(
                common::base_count(&reloaded),
                bases_before + 1,
...
fn a_placed_base_rebinds_every_object_it_brings() {
    ...
    for entry in &blueprint.characters {
        assert_eq!(
            ps_core::domain::world::entry_character_data(entry)
                .map(|data| ps_core::props::guid_to_uuid(&data.group_id)),
            Some(Uuid::nil()),
    ...
    assert_eq!(
        ps_core::props::guid_to_uuid(&placed_raw.group_id_belong_to),
        guild_id,
        "the placed base camp must belong to the target guild"
    );
    assert_eq!(
        ps_core::props::guid_to_uuid(&placed_raw.id),
        placed_base,
    ...
    for entry in ps_core::domain::world::character_map(&target.level).expect("characters") {
        let Some(instance_id) = ps_core::domain::world::entry_instance_id(entry) else { continue };
        ...
        assert_eq!(
            ps_core::domain::world::entry_save_parameter(entry)
                .and_then(|params| ps_core::props::get(params, &["OwnerPlayerUId"]))
...
fn every_capture_layer_gives_the_placed_base_a_worker_container() {
    ...
    let (_, source_container) =
        ps_core::domain::guild::base_guild_and_container(base_camp_entry(&source, source_base))
...
fn a_merge_drops_only_the_container_that_is_both_unnamed_and_empty() {
    for (layer, options, expected_dropped, expected_kept_for_pals) in [
        ...
        let reachable: BTreeSet<Uuid> = blueprint
            ...
            .collect();
        let named = |entry: &ps_core::ue::MapEntry| {
        ...
        let module_targets: BTreeSet<Uuid> =
            ps_core::domain::world::map_object_values(&session.level)
                .expect("map objects")
                .expect("MapObjectSaveData")
                .iter()
                .filter_map(|value| match value {
                    ps_core::ue::StructValue::Struct(properties) => Some(properties),
                    _ => None,
                })
