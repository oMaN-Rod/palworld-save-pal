...
use ps_core::domain::{guild, guild_tail, world};
use ps_core::dto::guild::GuildLabResearchInfo;
...
fn guild_details_load_real_base_lab_research_and_guild_chest() {
    ...
    assert_eq!(pal_container.player_uid, ps_core::props::EMPTY_UUID);
...
fn update_lab_research_with_no_lab_data_is_a_silent_no_op() {
    ...
    let extra_entry = MapEntry {
        key: Property::Struct(StructValue::Guid(ps_core::props::uuid_to_guid(guild_id))),
...
fn raw_tail_bytes(
    session: &SaveSession,
    guild_id: Uuid,
) -> ps_core::ue::games::palworld::PalGroupVariant {
