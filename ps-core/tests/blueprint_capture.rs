...
use ps_core::domain::blueprint::{capture, scrub, CaptureOptions};
use ps_core::domain::guild;
use uuid::Uuid;
...
fn dynamic_items_are_limited_to_those_the_containers_reference() {
    ...
    let all_dynamic = ps_core::domain::world::dynamic_item_values(&session.level)
...
fn asking_for_workers_does_not_capture_caged_pals() {
    ...
    let base_entry = ps_core::domain::world::base_camp_map(&session.level)
        .expect("base camp map")
        .and_then(|entries| entries.iter().find(|entry| ps_core::props::as_uuid(&entry.key) == Some(base_id)))
...
fn a_layer_without_worker_pals_captures_the_container_but_none_of_its_pals() {
    ...
    let base_entry = ps_core::domain::world::base_camp_map(&session.level)
        .expect("base camp map")
        .and_then(|entries| {
            entries.iter().find(|entry| ps_core::props::as_uuid(&entry.key) == Some(base_id))
        })
        .expect("fixture base camp entry must exist");
    let (_guild_id, worker_container_id) = guild::base_guild_and_container(base_entry)
        .expect("fixture base must resolve a worker container");
    let source_entry = ps_core::domain::world::character_container_map(&session.level)
