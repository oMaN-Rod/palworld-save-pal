//! The blueprint's wire form: a synthetic `Save<Palworld>`. `.psbp` is that `Save`'s
...
pub fn to_psbp_bytes(blueprint: &BaseBlueprint) -> Result<Vec<u8>, CoreError> {
    let save = to_save(blueprint)?;
    let body = savio::write_sav_bytes(&save)?;

    let mut out = Vec::with_capacity(PSBP_MAGIC.len() + 4 + body.len());
    out.extend_from_slice(PSBP_MAGIC);
...
pub fn from_psbp_bytes(bytes: &[u8]) -> Result<BaseBlueprint, CoreError> {
    ...
    version_bytes.copy_from_slice(&bytes[PSBP_MAGIC.len()..prefix]);
    let version = u32::from_le_bytes(version_bytes);
    if version > SCHEMA_VERSION {
        return Err(CoreError::Parse(format!(
            "psbp blueprint schema version {version} is newer than supported version {SCHEMA_VERSION}"
        )));
    }
