...
/// PS_GAMEPASS_PACKAGES_ROOT lets tests point at a synthetic tree instead.
fn default_packages_root() -> PathBuf {
    if let Ok(root) = std::env::var("PS_GAMEPASS_PACKAGES_ROOT") {
...
/// `backups` under the app root; override with PS_BACKUPS_ROOT.
pub fn backups_root() -> PathBuf {
    std::env::var("PS_BACKUPS_ROOT")
        .map(PathBuf::from)
...
mod tests {
    ...
    fn backups_root_honors_env_override() {
        ...
        std::env::set_var("PS_BACKUPS_ROOT", &override_path);
        let resolved = backups_root();
        std::env::remove_var("PS_BACKUPS_ROOT");
    ...
    fn find_container_dir_honors_packages_root_env_override() {
        ...
        std::env::set_var("PS_GAMEPASS_PACKAGES_ROOT", temp.path());
        let resolved = find_container_dir();
        std::env::remove_var("PS_GAMEPASS_PACKAGES_ROOT");
