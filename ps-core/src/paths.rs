...
//! the webview crashes. Instead the shell exports `PS_APP_ROOT`, and every
//! cwd-relative consumer resolves against it.

use std::ffi::OsString;
use std::path::PathBuf;

/// `PS_APP_ROOT` when the desktop shell exported it, else the process cwd
/// (web-server mode, tests, and unpackaged runs that never set it).
pub fn app_root() -> PathBuf {
    resolve_app_root(std::env::var_os("PS_APP_ROOT"))
}
...
mod tests {
    ...
    fn falls_back_to_cwd_when_empty() {
        // An exported-but-empty PS_APP_ROOT must NOT become PathBuf("") (which
