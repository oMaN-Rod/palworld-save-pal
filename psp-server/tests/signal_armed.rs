...
const RELINK_BACKOFF_VAR: &str = "PSP_SIGNAL_MEET_RELINK_BACKOFF_MS";
...
async fn seed_device(server: &common::TestServer, device_id: &str, secret: &[u8; 32]) {
    psp_db::signal_devices::insert_device(
        &*server.handle.app.driver,
        &psp_db::signal_devices::SignalDevice {
            device_id: device_id.to_string(),
            secret_hex: psp_server::signal::crypto::hex32(secret),
...
async fn an_armed_desktop_serves_a_paired_device_and_ignores_the_rest() {
    ...
    let identity = psp_db::meta::get(&*server.handle.app.driver, IDENTITY_SECRET_KEY)
        .await
        .unwrap()
    ...
    assert_eq!(
        psp_db::meta::get(&*server.handle.app.driver, ARMED_KEY)
            .await
            .unwrap()
            .as_deref(),
        Some("1")
...
async fn a_desktop_left_armed_arms_itself_at_startup() {
    ...
    let db_path = temp_dir.path().join("psp-rs.db");
    let identity = [0x2fu8; 32];
    {
        let driver = psp_db::SqlxSqliteDriver::new(psp_db::open(&db_path).await.unwrap());
        psp_db::meta::set(
            &driver,
            IDENTITY_SECRET_KEY,
            &psp_server::signal::crypto::hex32(&identity),
        )
        .await
        .unwrap();
        psp_db::meta::set(&driver, ARMED_KEY, "1").await.unwrap();
    }
    let ui_dir = temp_dir.path().join("ui");
    std::fs::create_dir_all(&ui_dir).unwrap();

    let handle = psp_server::start_server(psp_server::ServerConfig {
...
async fn block_meta_writes(db: &dyn psp_db::DbDriver) {
    for statement in [
...
async fn allow_meta_writes(db: &dyn psp_db::DbDriver) {
    for statement in [
...
async fn restoring_an_already_armed_desktop_leaves_one_link() {
    ...
    let _env = BrokerEnvGuard::acquire(&broker.url()).await;
    let server = common::start_test_server().await;
    psp_db::meta::set(&*server.handle.app.driver, ARMED_KEY, "1")
        .await
        .unwrap();
