...
use axum::Router;
use base64::Engine;
use psp_server::signal::crypto::{derive_keys, open, seal};
...
const PAIRING_URL_PREFIX: &str = "https://palworldsavepal.app/signal#v1.";
...
const DEVICE_ACK_TIMEOUT_VAR: &str = "PSP_SIGNAL_DEVICE_ACK_TIMEOUT_MS";
...
async fn next_frame_seq(
    rx: &mut tokio::sync::watch::Receiver<Option<psp_app::live::LiveFrame>>,
) -> u64 {
...
async fn wait_for_a_device_row(
    driver: &dyn psp_db::DbDriver,
) -> Vec<psp_db::signal_devices::SignalDevice> {
    let deadline = tokio::time::Instant::now() + Duration::from_secs(10);
    loop {
        let devices = psp_db::signal_devices::list_devices(driver).await.unwrap();
        if !devices.is_empty() {
            return devices;
        }
        assert!(
...
async fn pairing_persists_no_device_when_the_guest_never_acks() {
    ...
    assert!(
        psp_db::signal_devices::list_devices(&*handshake.server.handle.app.driver)
            .await
            .unwrap()
            .is_empty(),
...
async fn meta(server: &common::TestServer, key: &str) -> Option<String> {
    psp_db::meta::get(&*server.handle.app.driver, key)
        .await
        .unwrap()
}

async fn device_names(server: &common::TestServer) -> Vec<String> {
    psp_db::signal_devices::list_devices(&*server.handle.app.driver)
        .await
        .unwrap()
        .into_iter()
