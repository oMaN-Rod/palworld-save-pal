...
const PRODUCTION_BROKER_URL: &str = "wss://palworldsavepal.app";
...
pub fn default_broker_url() -> String {
    std::env::var("PSP_SIGNAL_BROKER_URL").unwrap_or_else(|_| PRODUCTION_BROKER_URL.to_string())
}
...
mod tests {
    ...
    async fn default_broker_url_uses_the_compiled_in_default_when_unset() {
        let _env_guard = SignalEnvGuard::acquire(&[("PSP_SIGNAL_BROKER_URL", None)]).await;
        assert!(std::env::var("PSP_SIGNAL_BROKER_URL").is_err());
        assert_eq!(default_broker_url(), "wss://palworldsavepal.app");
    }

    #[test]
    fn turn_credential_url_swaps_schemes() {
        assert_eq!(
            turn_credential_url("wss://palworldsavepal.app"),
            "https://palworldsavepal.app/signal/turn"
        );
        assert_eq!(
    ...
    fn turn_response_parses_into_ice_servers() {
        let json = r#"{"urls":["turn:t.example:3478?transport=udp","turns:t.example:443?transport=tcp"],"username":"1700000000:psp","credential":"abc=","ttl_seconds":3600}"#;
        let servers = parse_turn_response(json).unwrap();
        assert_eq!(servers.len(), 1);
        assert_eq!(servers[0].urls.len(), 2);
        assert_eq!(servers[0].username.as_deref(), Some("1700000000:psp"));
