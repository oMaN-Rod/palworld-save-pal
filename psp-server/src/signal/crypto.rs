...
//! for Signal pairing. The TypeScript port in `psp-ui/src/lib/signal/crypto.ts`
...
mod tests {
    ...
    fn load_vectors() -> VectorsFile {
        const RAW: &str = include_str!("../../../psp-ui/src/lib/signal/vectors.json");
    ...
    /// `cargo test -p psp-server --lib -- --ignored print_vectors_fixture --nocapture`
