...
/** `SaveFile`'s declared type omits the wire's `session_id`; it is present at runtime. */
type SaveFileWithSession = SaveFile & { session_id?: string };

function currentSessionId(): string | null {
	const saveFile = getAppState().saveFile as SaveFileWithSession | undefined;
	return saveFile?.session_id ?? null;
}
