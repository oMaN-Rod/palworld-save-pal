...
// `setLocale` must run before `persist`: it only reloads when the locale actually
// changes, so setting it first makes the backend's settings echo (which calls
// `setLocale` with the default `reload: true`) a no-op.
...
// The settings echo no longer applies the backend's language, so the edit has to
// switch the locale itself; `switchLocale` persists on its way, leaving
// `persistAll` for the case where only the other fields changed.
