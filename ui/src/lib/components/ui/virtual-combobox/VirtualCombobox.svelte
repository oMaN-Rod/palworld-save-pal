...
<div class={labelClass} bind:this={containerRef}>
	...
	>
		...
		{#if isOpen}
			<div
				id={listboxId}
				class="bg-surface-900 border-surface-600 select-popup rounded-xs absolute left-0 right-0 mt-3 border shadow-lg"
				role="listbox"
			>
				<VirtualList
					width="100%"
					height={Math.min(filteredOptions.length * optionHeight, 240)}
