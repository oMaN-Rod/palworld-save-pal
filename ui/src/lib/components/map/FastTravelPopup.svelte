	...
	<div class="pointer-events-auto space-y-3">
		<div class="border-b pb-3">
			<div class="flex items-start gap-2">
				<Icon icon="tabler:navigation" class="text-primary mt-1 h-5 w-5 shrink-0" />
		...
		<div class="space-y-2">
			{#if point.unlocked !== undefined}
				<div class="flex items-center gap-2">
					{#if point.unlocked}
						<Icon icon="tabler:lock-open" class="h-4 w-4 shrink-0 text-green-400" />
						<span class="text-sm text-green-400">{m.unlocked()}</span>
					{:else}
						<Icon icon="tabler:lock" class="h-4 w-4 shrink-0 text-red-400" />
