...
const structure = (over = {}) => ({
	instance_id: 'i1', map_object_id: 'BlastFurnace',
	x: 0, y: 0, z: 0, yaw: 0,
	scale_x: 1, scale_y: 1, scale_z: 1,
	hp_current: 850, hp_max: 1000, build_player_uid: 'ABC-123',
	...over
});
const footprints = { BlastFurnace: { sx: 168, sy: 100, sz: 82, ox: 0, oy: 0, oz: 0, typeA: 'Product' } };
const buildings = { BlastFurnace: { localized_name: 'Blast Furnace', description: 'Smelts ore.', rank: 2, icon: 't_icon' } } as never;
...
describe('structureInfo', () => {
	...
	it('falls back to the raw map_object_id when the building is unknown', () => {
		const info = structureInfo(structure({ map_object_id: 'DamagableRock0001' }), footprints, buildings, summaries);
		expect(info.name).toBe('DamagableRock0001');
	});

	it('falls back to the Other type when the footprint is unknown', () => {
		const info = structureInfo(structure({ map_object_id: 'Nope' }), footprints, buildings, summaries);
	...
	it('resolves the builder nickname case-insensitively', () => {
		const info = structureInfo(structure({ build_player_uid: 'abc-123' }), footprints, buildings, summaries);
		expect(info.builder).toBe('Omar');
	});

	it('leaves the builder undefined when the uid is unknown', () => {
		const info = structureInfo(structure({ build_player_uid: 'ZZZ' }), footprints, buildings, summaries);
