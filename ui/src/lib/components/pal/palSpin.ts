// Turntable model for the Pal viewer: an idle rotation the pointer can take over
// and hand back. Free of three.js and the DOM so its feel is testable without a
// GL context -- the viewer feeds it frame times and pointer x, and reads `angle`.
const TAU = Math.PI * 2;

export const REVOLUTION_MS = 20000;
export const AUTO_SPIN_RAD_PER_MS = TAU / REVOLUTION_MS;
// A ~300 px drag turns the Pal all the way round, so a modal-width sweep is a
// full inspection.
export const DRAG_RAD_PER_PX = TAU / 300;
// Past a revolution per second a flick reads as blur, and trackpads report far
// larger deltas than that.
...
export class PalSpin {
	...
	advance(dtMs: number): number {
		...
		// Exponential rather than a fixed step so the return to idle is frame-rate
	...
	pointerDown(x: number, tMs: number): void {
		...
		this.#lastX = x;
		this.#lastT = tMs;
		// Otherwise the last flick's leftover velocity is handed back at release,
		// spinning a Pal the user grabbed to hold still.
		this.velocity = 0;
	}
