			...
			{#each chain.sources as src}
				...
				>
					...
					<span class="chip px-1.5 py-0 text-[10px] {meta.cls} shrink-0">
						<Icon icon={srcIcon} size={9} class="inline" />{meta.label()}
					</span>
				</div>
			{/each}
		</div>
			...
			{#each chain.steps as step, i}
				<div class="breed-row rounded-sm bg-surface-900/30 border-surface-700/20 border p-2">
					...
					<PalSlot
						...
						size="md"
					/>
					<span class="breed-op"><Icon icon="tabler:plus" size={16} class="text-primary-400" /></span>
					<PalSlot
						tribe={step.parent_b}
						...
						size="md"
					/>
					<span class="breed-op"><Icon icon="tabler:arrow-right" size={18} class="text-primary-400" /></span>
					<PalSlot
						tribe={step.child}
