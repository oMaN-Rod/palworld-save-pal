<script lang="ts">
	...
	let workSuitabilities: Record<string, number> = $state({});
	...
	async function loadPalWorkSuitabilities() {
		if (pal) {
			const palData = await palsData.searchByLocalizedName(pal.name);
			if (palData) {
				workSuitabilities = palData.work_suitability;
			}
		}
	}

	$effect(() => {
		loadPalWorkSuitabilities();
	});
</script>
