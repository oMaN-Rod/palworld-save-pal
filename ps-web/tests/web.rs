...
async fn get_version_round_trips_through_the_callback() {
    ...
    ps_web::set_emit_callback(cb.as_ref().unchecked_ref::<js_sys::Function>().clone());
    cb.forget();

    ps_web::dispatch_frame(r#"{"type":"get_version","data":null}"#.to_string())
        .await
        .unwrap();
...
async fn world_save_round_trips_through_wasm_as_bytes() {
    ...
    ps_web::stage_gvas("level", "", gvas.to_vec()).unwrap();
    ps_web::load_staged_gvas("world1".to_string()).await.unwrap();
    ...
    let manifest = ps_web::export_gvas_manifest().await.unwrap();
    ...
    let out = ps_web::export_gvas_file("Level.sav".to_string()).await.unwrap();
    assert_eq!(out, gvas.to_vec(), "round-trip GVAS is byte-identical");
}

/// wasm32 links no Oodle codec, so `.sav`/`.psbp` containers can only be written
...
async fn oodle_bridge_marshals_bytes_and_length_between_the_engine_and_js() {
    ...
    ps_web::set_oodle_bridge(compress.unchecked_into(), decompress.unchecked_into());

    let gvas = include_bytes!("fixtures/world1-level.gvas");
    let save = ps_core::savio::read_gvas_bytes(gvas).expect("fixture parses");
    let sav = ps_core::savio::write_sav_bytes(&save).expect("writes through the bridge");
    ...
    ps_core::savio::read_sav_bytes(&sav).expect("reads back through the bridge");
    let observed = js_sys::Reflect::get(&js_sys::global(), &"__oodle".into()).unwrap();
    assert_eq!(
...
async fn sql_bridge_marshals_and_migrations_call_through() {
    ...
    ps_web::set_sql_bridge(exec.clone().unchecked_into(), query.unchecked_into());
    ps_web::run_migrations().await.unwrap();
