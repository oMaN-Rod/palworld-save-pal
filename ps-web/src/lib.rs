...
use ps_app::dispatcher::{dispatch, HandlerCtx, NullExtRouter, SessionAttachment};
use ps_app::emitter::Emitter;
use ps_app::envelope::Envelope;
use ps_app::handlers::plugins::seed_bundled_plugins;
use ps_app::handlers::web_save::{handle_load_save_gvas_bytes, LoadSaveGvasBytes, StagedGvas};
use ps_app::{AppConfig, AppState, SessionStore};
...
pub async fn run_migrations() -> Result<(), JsValue> {
    ps_db::run_migrations(&opfs_driver::OpfsSqlDriver)
        .await
...
pub async fn export_gvas_manifest() -> Result<JsValue, JsValue> {
    with_session(|session| {
        let manifest = ps_app::handlers::web_save::export_manifest(session);
...
pub async fn export_gvas_file(name: String) -> Result<Vec<u8>, JsValue> {
    with_session(|session| {
        ps_app::handlers::web_save::export_file(session, &name)
            .map_err(|e| JsValue::from_str(&e.to_string()))
    })
    .await
}
...
pub fn sav_to_json(sav_bytes: &[u8]) -> Result<String, JsValue> {
    ps_core::convert::sav_to_json_string(sav_bytes)
...
pub fn json_to_sav(json: &str) -> Result<Vec<u8>, JsValue> {
    ps_core::convert::json_to_sav_bytes(json.as_bytes())
        .map_err(|error| JsValue::from_str(&error.to_string()))
}
...
async fn with_session<T>(
    f: impl FnOnce(&ps_core::session::SaveSession) -> Result<T, JsValue>,
) -> Result<T, JsValue> {
...
async fn run_with_ctx(op: Op) -> Result<(), JsValue> {
    ...
    let holds_own_session_lock = match &op {
        Op::Frame(envelope) => !matches!(
            ps_app::messages::MessageType::from_wire(&envelope.message_type),
            Some(
                ps_app::messages::MessageType::ReattachSession
                    | ps_app::messages::MessageType::EjectSession
            )
        ),
