...
async fn update_persists_everything_except_save_dir() {
    ...
    drop(pool);
    drop(db);
    let reopened = ps_db::open(&db_path).await.unwrap();
    let reopened_db = ps_db::SqlxSqliteDriver::new(reopened);
...
async fn update_on_empty_db_creates_row_with_default_save_dir() {
    ...
    assert_eq!(created.save_dir, ps_db::settings::default_steam_save_dir());
}
