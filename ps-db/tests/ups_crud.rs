...
async fn seed_pal(db: &ps_db::SqlxSqliteDriver) -> ps_db::ups::UpsPalRecord {
    ps_db::ups::add_pal(
        db,
        ps_db::ups::NewUpsPal {
...
async fn update_pal_syncs_columns_both_directions() {
    ...
    let updated = ps_db::ups::update_pal(&db, pal.id, &updates)
        .await
        .unwrap()
        .unwrap();
    ...
    assert!(ps_db::ups::update_pal(&db, 9999, &serde_json::Map::new())
        .await
        .unwrap()
        .is_none());
}
...
async fn clone_delete_and_nuke() {
    ...
    let clone = ps_db::ups::clone_pal(&db, pal.id, &game_data())
        .await
        .unwrap()
        .unwrap();
    ...
    let original = ps_db::ups::get_pal_by_id(&db, pal.id)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(original.clone_count, 1);

    let deleted = ps_db::ups::delete_pals(&db, &[pal.id, 9999], &game_data())
        .await
        .unwrap();
    assert_eq!(deleted, 1);

    let nuked = ps_db::ups::nuke_all_pals(&db, &game_data()).await.unwrap();
...
async fn tag_rename_and_delete_propagate_to_pals() {
    let (db, _) = test_db().await;
    let pal = seed_pal(&db).await;
    let tag = ps_db::ups::create_or_update_tag(&db, "shiny", None, Some("#0f0"))
        .await
        .unwrap();

    let mut updates = serde_json::Map::new();
    updates.insert("name".into(), serde_json::json!("sparkly"));
    ps_db::ups::update_tag(&db, tag.id, &updates)
        .await
        .unwrap()
        .unwrap();
    let after_rename = ps_db::ups::get_pal_by_id(&db, pal.id)
        .await
        .unwrap()
        .unwrap();
    assert_eq!(after_rename.tags, serde_json::json!(["sparkly"]));

    assert!(ps_db::ups::delete_tag(&db, tag.id).await.unwrap());
    let after_delete = ps_db::ups::get_pal_by_id(&db, pal.id)
        .await
        .unwrap()
        .unwrap();
...
async fn collection_update_and_delete_detach_pals() {
    ...
    ps_db::ups::update_pal(&db, pal.id, &updates)
        .await
        .unwrap();

    let mut collection_updates = serde_json::Map::new();
    collection_updates.insert("is_favorite".into(), serde_json::json!(true));
    let updated = ps_db::ups::update_collection(&db, collection.id, &collection_updates)
        .await
        .unwrap()
        .unwrap();
    assert!(updated.is_favorite);

    assert!(ps_db::ups::delete_collection(&db, collection.id)
        .await
        .unwrap());
    let detached = ps_db::ups::get_pal_by_id(&db, pal.id)
        .await
        .unwrap()
        .unwrap();
