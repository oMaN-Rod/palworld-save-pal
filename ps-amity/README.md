...
WebSocket that PalStudio connects to for live game state.
...
- `scripts/setup-workspace.ps1 [-Root <dir>]` — one-time dev setup: clones the pinned UE4SS
  fork into a CMake workspace and junctions this directory into it as `PSAmity`. The mod's
  CMakeLists.txt links the `UE4SS` target, so it only configures as a subdirectory of that
  workspace.
- `scripts/build.ps1 [-Root <dir>] [-Targets PSAmity,AmityCoreTests]` — build the mod DLL,
...
- `tools/probe.ts` — bun client for driving the bridge by hand: every read and write op has
  ...
  socket. It discovers the running instance from `%LOCALAPPDATA%\Pal\Saved\PSAmity\endpoints\`
...
The mod's startup resolution report (`[PSAmity] resolve ...` lines in UE4SS.log) checks
