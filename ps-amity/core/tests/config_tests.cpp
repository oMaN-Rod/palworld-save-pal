...
TEST_CASE("parse_ini reads the bridge section") {
    auto cfg = amity::parse_ini(
        ...
        "token = s3cr3t\n"
        "bind = 0.0.0.0\n");
    CHECK(cfg.name == "Dedicated - world4");
    CHECK(cfg.port == 8788);
...
TEST_CASE("apply_env_overrides wins over file values") {
    auto cfg = amity::parse_ini("[bridge]\nport = 8788\nname = From File\n");
    amity::apply_env_overrides(cfg, env_from({{"PSAMITY_PORT", "9999"},
                                              {"PSAMITY_NAME", "From Env"},
                                              {"PSAMITY_TOKEN", "envtoken"},
                                              {"PSAMITY_BIND", "0.0.0.0"}}));
    CHECK(cfg.port == 9999);
