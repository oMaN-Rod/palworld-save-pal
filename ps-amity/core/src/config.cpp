...
void apply_env_overrides(BridgeConfig& cfg, const EnvLookup& lookup) {
    if (auto v = lookup("PSAMITY_NAME"); v && !v->empty()) cfg.name = *v;
    if (auto v = lookup("PSAMITY_TOKEN"); v) cfg.token = *v;
    if (auto v = lookup("PSAMITY_BIND"); v && !v->empty()) cfg.bind = *v;
    if (auto v = lookup("PSAMITY_PORT"); v) {
        int port = 0;
