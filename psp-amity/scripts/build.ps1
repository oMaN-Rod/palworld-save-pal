...
try {
    ...
    $dll = Get-ChildItem -Recurse -Filter "PSPAmity.dll" (Join-Path $Root "build") |
        Where-Object { $_.FullName -like "*$Config*" } | Select-Object -First 1
    if (-not $dll) { throw "PSPAmity.dll not found under $Root\build for config $Config" }
