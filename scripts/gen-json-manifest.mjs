...
// depend on the caller's working directory (bun runs scripts with cwd = ps-ui/).
const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const srcDir = resolve(repoRoot, 'data/json');
const destDir = resolve(repoRoot, 'ps-ui/static/data/json');
...
console.log(`web assets: ${files.length} json files + manifest → ps-ui/static/data/json`);
