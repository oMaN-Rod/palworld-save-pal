...
fn an_out_of_range_level_is_refused() {
    ...
    assert!(
        !err.contains("attempt to index a ps.pal value"),
...
/// A base worker pal PalStudio itself created carries `OwnerPlayerUId` as the nil guid
...
fn seed_base_worker(h: &mut support::Harness) -> uuid::Uuid {
    ...
    let mut pairs: Vec<(uuid::Uuid, uuid::Uuid)> = {
        let summaries =
            ps_core::domain::pal::pal_summaries(h.session(), &game_data).expect("summaries build");
    ...
    for (guild_id, base_id) in pairs {
        ps_core::domain::guild::get_guild_details(h.session_mut(), &game_data, guild_id)
            .expect("guild details load")
            .expect("the guild exists");
        let added = ps_core::domain::pal::add_guild_pal(
            h.session_mut(),
            &game_data,
            guild_id,
            base_id,
        ...
        if let Some(dto) = added {
            let raw_owner = ps_core::domain::world::character_map(&h.session().level)
                .expect("the character map")
                .iter()
                .find(|e| {
                    ps_core::domain::world::entry_instance_id(e) == Some(dto.instance_id)
                })
