...
use ps_core::dto::container::ItemContainerDto;
...
fn game_data() -> &'static ps_core::gamedata::GameData {
    static GAME_DATA: std::sync::OnceLock<ps_core::gamedata::GameData> =
    ...
    GAME_DATA.get_or_init(|| {
        let data_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .join("data/json");
        ps_core::gamedata::GameData::load(&data_dir).expect("game data is checked in")
    })
}

/// The container as `ps-core` reads it back off the save, with no part of the
...
fn slot_in_the_save(
    ...
    container_id: Uuid,
    slot_index: i32,
) -> Option<ps_core::dto::container::ItemContainerSlotDto> {
...
fn dynamic_item_entry_exists(harness: &support::Harness, local_id: Uuid) -> bool {
    ps_core::domain::world::build_dynamic_item_index(&harness.session().level)
...
fn scan_slots(
    ...
    accept: impl Fn(&ps_core::dto::container::ItemContainerSlotDto) -> bool,
) -> Option<(Uuid, i32)> {
    ...
    for id in container_ids(harness) {
        ...
        let level = &session.level;
        let caches = &mut session.caches;
        let Some(dto) = ps_core::domain::containers::read_item_container(
