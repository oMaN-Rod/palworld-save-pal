use ps_plugin::manifest::{Capability, Manifest, ManifestError, ParamValue};
...
fn manifest_with_param(param_json: &str) -> Result<ps_plugin::manifest::Manifest, ps_plugin::manifest::ManifestError> {
    let json = format!(
        r#"{{
            "id": "test.plugin",
            "api_version": 1,
    ...
    ps_plugin::manifest::Manifest::parse(&json)
}
...
fn an_entity_param_coerces_a_string_id() {
    ...
    assert_eq!(coerced, vec![("who".to_string(), ps_plugin::manifest::ParamValue::Text("abc-123".to_string()))]);
...
fn a_multiselect_param_coerces_an_array_of_strings() {
    ...
    assert_eq!(
        ...
        vec![(
            "ids".to_string(),
            ps_plugin::manifest::ParamValue::List(vec!["a".to_string(), "b".to_string()])
        )]
    );
    ...
    assert_eq!(empty, vec![("ids".to_string(), ps_plugin::manifest::ParamValue::List(Vec::new()))]);
...
fn every_sp1_param_type_still_parses_and_coerces() {
    ...
    let manifest = ps_plugin::manifest::Manifest::parse(json).expect("the SP1 shape must still parse");
