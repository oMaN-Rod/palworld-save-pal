...
use ps_plugin::context::{LogLine, RunContext};
use ps_plugin::host;
use ps_plugin::manifest::{Capability, Manifest};
use ps_plugin::runtime::{self, RunOutcome, RunRequest, RunServices};
use ps_plugin::sandbox::{Cancel, Limits, Sandbox};
...
pub struct Harness {
    ...
    progress: Option<ps_core::progress::ProgressSink>,
    dto_flush_count: u64,
...
impl Harness {
    ...
    pub fn with_progress(mut self, sink: ps_core::progress::ProgressSink) -> Self {
    ...
    pub fn run(&mut self, source: &str) -> (RunStatus, Option<String>) {
        ...
        let mut ctx = RunContext::new(
            ...
            ps_plugin::manifest::SUPPORTED_API_VERSION,
