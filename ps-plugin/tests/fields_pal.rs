...
fn pal_fields_declares_guild_id_and_base_id() {
    let names: Vec<&str> = ps_plugin::PAL_FIELDS.iter().map(|f| f.name).collect();
    assert!(names.contains(&"guild_id"), "{names:?}");
    assert!(names.contains(&"base_id"), "{names:?}");
    for name in ["guild_id", "base_id"] {
        let spec = ps_plugin::PAL_FIELDS.iter().find(|f| f.name == name).expect("present");
        assert_eq!(spec.access, ps_plugin::Access::ReadOnly, "{name} has no PalDto field to write into");
    }
}
...
/// `nickname` is declared `string|nil`, so an author reading `ps.lua` sees a
...
/// (`ps-core/src/domain/pal.rs:614`), so a direct write here could silently
...
/// `work_suitability`'s key order is a wire contract (`ps-core/src/dto/pal.rs`
