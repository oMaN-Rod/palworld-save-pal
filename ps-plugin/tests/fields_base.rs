...
fn a_base_id(harness: &support::Harness) -> uuid::Uuid {
    ...
    ps_core::props::as_uuid(&entry.key).expect("a base entry is keyed by its uuid")
}

/// The base as `ps-core` reads it back off the save, with no part of the
...
fn base_in_the_save(
    ...
    id: uuid::Uuid,
) -> ps_core::dto::guild::BaseDto {
    let data_dir = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("ps-plugin has a parent directory")
        .join("data/json");
    let game_data = ps_core::gamedata::GameData::load(&data_dir).expect("game data is checked in");
    let guild_ids: Vec<uuid::Uuid> = harness.session().guild_summary_order.clone();
    for guild_id in guild_ids {
        let Some(dto) =
            ps_core::domain::guild::get_guild_details(harness.session_mut(), &game_data, guild_id)
...
/// state `ps-core`'s own `build_guild_dto` answers `(None, None, None)` for,
...
fn break_the_last_bases_record(harness: &mut support::Harness) -> uuid::Uuid {
    let entries = ps_core::domain::world::base_camp_map_mut(&mut harness.session_mut().level)
        .expect("the corpus fixture has a base camp map")
        .expect("the corpus fixture has a base camp map");
    let entry = entries.last_mut().expect("the corpus fixture has bases");
    let id = ps_core::props::as_uuid(&entry.key).expect("a base entry is keyed by its uuid");
    ps_core::props::struct_props_mut(&mut entry.value)
        .expect("a base entry's value is a struct")
        .0
        .shift_remove(&ps_core::ue::PropertyKey::from("RawData"));
    id
}
