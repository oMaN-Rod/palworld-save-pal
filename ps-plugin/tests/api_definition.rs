...
use ps_plugin::host::api_def::{api_definition, ApiField, ApiType};
use ps_plugin::manifest::Capability;
use ps_plugin::Access;
...
fn every_pal_field_row_appears_in_the_api_definition() {
    ...
    for spec in ps_plugin::PAL_FIELDS {
...
fn every_described_pal_field_exists_in_the_table() {
    ...
    for field in handle.fields {
        assert!(
            ps_plugin::PAL_FIELDS.iter().any(|s| s.name == field.name),
...
fn every_player_field_row_appears_in_the_api_definition() {
    ...
    for spec in ps_plugin::PLAYER_FIELDS {
        let described = handle.fields.iter().find(|f| f.name == spec.name);
        assert!(described.is_some(), "{} is in the table but not the definition", spec.name);
        let described = described.expect("checked above");
        assert_eq!(described.ty, spec.ty, "{} disagrees on type", spec.name);
...
fn every_described_player_field_exists_in_the_table() {
    ...
    for field in handle.fields {
        assert!(
            ps_plugin::PLAYER_FIELDS.iter().any(|s| s.name == field.name),
            "{} is described but has no table row",
            field.name
        );
    }
...
fn every_player_field_row_reads_back_at_its_declared_type() {
    ...
    assert_eq!(ps_plugin::PLAYER_FIELDS.len(), seen.len(), "every row must be probed exactly once");
    assert_eq!(
        read_as_nil,
        Vec::<&str>::new(),
...
fn every_guild_field_row_appears_in_the_api_definition() {
    ...
    for spec in ps_plugin::GUILD_FIELDS {
        let described = handle.fields.iter().find(|f| f.name == spec.name);
        assert!(described.is_some(), "{} is in the table but not the definition", spec.name);
        let described = described.expect("checked above");
        assert_eq!(described.ty, spec.ty, "{} disagrees on type", spec.name);
...
fn every_described_guild_field_exists_in_the_table() {
    ...
    for field in handle.fields {
        assert!(
            ps_plugin::GUILD_FIELDS.iter().any(|s| s.name == field.name),
            "{} is described but has no table row",
            field.name
        );
    }
...
fn every_base_field_row_appears_in_the_api_definition() {
    ...
    for spec in ps_plugin::BASE_FIELDS {
        let described = handle.fields.iter().find(|f| f.name == spec.name);
        assert!(described.is_some(), "{} is in the table but not the definition", spec.name);
        let described = described.expect("checked above");
        assert_eq!(described.ty, spec.ty, "{} disagrees on type", spec.name);
...
fn every_described_base_field_exists_in_the_table() {
    ...
    for field in handle.fields {
        assert!(
            ps_plugin::BASE_FIELDS.iter().any(|s| s.name == field.name),
            "{} is described but has no table row",
            field.name
        );
    }
...
fn every_guild_field_row_reads_back_at_its_declared_type() {
    ...
    assert_eq!(ps_plugin::GUILD_FIELDS.len(), seen.len(), "every row must be probed exactly once");
    assert_eq!(
        read_as_nil,
        Vec::<&str>::new(),
...
fn every_base_field_row_reads_back_at_its_declared_type() {
    ...
    assert_eq!(ps_plugin::BASE_FIELDS.len(), seen.len(), "every row must be probed exactly once");
    assert_eq!(
        read_as_nil,
        Vec::<&str>::new(),
...
fn every_container_field_row_appears_in_the_api_definition() {
    ...
    for spec in ps_plugin::CONTAINER_FIELDS {
        let described = handle.fields.iter().find(|f| f.name == spec.name);
        assert!(described.is_some(), "{} is in the table but not the definition", spec.name);
        let described = described.expect("checked above");
        assert_eq!(described.ty, spec.ty, "{} disagrees on type", spec.name);
...
fn every_described_container_field_exists_in_the_table() {
    ...
    for field in handle.fields {
        assert!(
            ps_plugin::CONTAINER_FIELDS.iter().any(|s| s.name == field.name),
            "{} is described but has no table row",
            field.name
        );
    }
...
fn every_slot_field_row_appears_in_the_api_definition() {
    ...
    for spec in ps_plugin::SLOT_FIELDS {
        let described = handle.fields.iter().find(|f| f.name == spec.name);
        assert!(described.is_some(), "{} is in the table but not the definition", spec.name);
        let described = described.expect("checked above");
        assert_eq!(described.ty, spec.ty, "{} disagrees on type", spec.name);
...
fn every_described_slot_field_exists_in_the_table() {
    ...
    for field in handle.fields {
        assert!(
            ps_plugin::SLOT_FIELDS.iter().any(|s| s.name == field.name),
            "{} is described but has no table row",
            field.name
        );
    }
...
fn every_container_field_row_reads_back_at_its_declared_type() {
    ...
    assert_eq!(ps_plugin::CONTAINER_FIELDS.len(), seen.len(), "every row must be probed exactly once");
    assert_eq!(
        read_as_nil,
        Vec::<&str>::new(),
...
fn every_slot_field_row_reads_back_at_its_declared_type() {
    ...
    assert_eq!(ps_plugin::SLOT_FIELDS.len(), seen.len(), "every row must be probed exactly once");
    assert_eq!(
        read_as_nil,
        Vec::<&str>::new(),
...
fn wrong_typed_literal(ty: &ApiType) -> Option<&'static str> {
    ...
    [("true", "boolean"), ("'ps_probe'", "string"), ("1.5", "float"), ("{}", "table")]
        .into_iter()
...
fn implausible_literal(ty: &ApiType) -> Option<&'static str> {
    Some(match ty {
        ...
        ApiType::Number => "1e40",
        ApiType::String => "'ps_implausible_value'",
        ApiType::Boolean => "false",
        ApiType::List(_) => "{ 'ps_implausible_value' }",
        ApiType::Map { .. } => "{ ps_implausible_key = -987654321 }",
        ApiType::Table => "{ ps_implausible_key = -987654321 }",
