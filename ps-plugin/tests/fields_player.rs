...
fn player_common_container(harness: &mut support::Harness) -> uuid::Uuid {
    ...
    let dto = ps_core::domain::player::get_player_details(
        harness.session_mut(),
        &game_data,
        uid,
...
fn effigy_script() -> String {
    format!(
        "local target\n\
         ...
         effigies[#effigies + 1] = 'ps_test_effigy_flag'\n\
...
fn only_the_rows_that_were_always_save_read_are_readable_without_the_players_capability() {
    ...
    assert_eq!(probed, ps_plugin::PLAYER_FIELDS.len(), "every row must be probed exactly once");
    assert_eq!(
