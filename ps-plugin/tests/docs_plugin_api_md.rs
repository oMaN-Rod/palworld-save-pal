...
use ps_plugin::host::fields::player::read_requires_players;
use ps_plugin::host::MAX_TABLE_NODES;
use ps_plugin::manifest::Capability;
use ps_plugin::status::RunStatus;
use ps_plugin::PLAYER_FIELDS;
use uuid::Uuid;

fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("ps-plugin has a parent directory")
        .to_path_buf()
}
...
fn game_data() -> &'static ps_core::gamedata::GameData {
    static GAME_DATA: std::sync::OnceLock<ps_core::gamedata::GameData> = std::sync::OnceLock::new();
    GAME_DATA.get_or_init(|| {
        ps_core::gamedata::GameData::load(&repo_root().join("data/json")).expect("game data is checked in")
    })
}
...
fn a_plain_slot(harness: &mut support::Harness) -> (Uuid, i32) {
    ...
    for id in container_ids {
        ...
        let Some(container) =
            ps_core::domain::containers::read_item_container(level, caches, game_data(), id, "", None)
        else {
            continue;
        };
...
fn the_worked_example_is_the_bundled_plugins_own_code_and_still_runs() {
    ...
    let bundled = read_normalized("ps-app/src/bundled/pst.cleanup/main.lua");
    assert!(
        bundled.contains(&sample),
        "the worked example no longer appears in ps-app/src/bundled/pst.cleanup/main.lua, \
...
fn node_count(value: &serde_json::Value) -> usize {
    ps_plugin::host::gamedata::count_nodes(value, 0)
...
fn every_widget_kind_is_documented() {
    let docs = docs();
    for kind in ps_plugin::manifest::WIDGET_KINDS {
...
fn every_entity_kind_is_documented() {
    let docs = docs();
    for kind in ps_plugin::manifest::ENTITY_KINDS {
