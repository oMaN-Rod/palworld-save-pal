...
const CHUNK_NAME: &std::ffi::CStr = c"=ps";
const PREFIX: &str = "ps:";
...
mod tests {
    ...
    fn a_positioned_message_splits_into_line_and_text() {
        let split = split_position("ps:12: unexpected symbol near '='");
    ...
    fn a_non_numeric_position_is_kept_whole() {
        let split = split_position("ps:abc: something");
        assert_eq!(split.line, None);
        assert_eq!(split.message, "ps:abc: something");
    }
}
