...
/// message goes back to `ps_host_trampoline`, which raises from its own C frame.
...
macro_rules! host_fn {
    ($name:ident, $body:path) => {
        ...
        pub unsafe fn $name(state: *mut ::ps_lua_sys::ffi::lua_State) {
            ...
            ::ps_lua_sys::ffi::lua_pushlightuserdata(state, dispatch);
            ::ps_lua_sys::ffi::lua_pushlightuserdata(state, body);
            ::ps_lua_sys::ffi::lua_pushlightuserdata(state, free);
            ::ps_lua_sys::ffi::lua_pushcclosure(
                state,
                ::ps_lua_sys::ffi::ps_host_trampoline,
                3,
            );
        }
    };
