...
use ps_core::domain::{containers, map_object, pal, world};
use ps_core::dto::summary::IsoDateTime;
...
pub(crate) fn read_container<'ctx>(
    ...
    container_id: Uuid,
) -> Option<&'ctx ps_core::dto::container::ItemContainerDto> {
...
unsafe fn push_iterator(state: *mut lua_State, body: HostFn, epoch: u64, extra: Option<&str>) {
    ...
    lua_pushcclosure(state, ps_host_trampoline, n);
}
...
pub(crate) fn iter_metatable_name(kind: DeleteWhereKind) -> &'static CStr {
    match kind {
        DeleteWhereKind::Player => c"ps.iter.player",
        DeleteWhereKind::Guild => c"ps.iter.guild",
        DeleteWhereKind::Pal => c"ps.iter.pal",
        DeleteWhereKind::MapObject => c"ps.iter.map_object",
    }
}
...
unsafe fn push_player_pals_factory(state: *mut lua_State, owner: Uuid) {
    ...
    lua_pushcclosure(state, ps_host_trampoline, 4);
}
