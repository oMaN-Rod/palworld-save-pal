...
fn metatable_name(kind: HandleKind) -> &'static CStr {
    match kind {
        HandleKind::Player => c"ps.player",
        HandleKind::Pal => c"ps.pal",
        HandleKind::Guild => c"ps.guild",
        HandleKind::Base => c"ps.base",
        HandleKind::Container => c"ps.container",
        HandleKind::Slot => c"ps.slot",
        HandleKind::MapObject => c"ps.map_object",
    }
}
...
fn handle_tostring(state: *mut lua_State) -> Result<c_int, HostError> {
    unsafe {
        ...
        if ptr.is_null() {
            super::marshal::push_str(state, "ps.handle");
            return Ok(1);
        }
        let handle = std::ptr::read(ptr);
