...
use ps_core::domain::{containers, guild, guild_tail, pal, player, world};
use ps_core::dto::container::ItemContainerSlotDto;
use ps_core::dto::guild::{BaseDto, GuildDto};
...
use ps_core::progress::{null_progress, ProgressSink};
use ps_core::props;
use ps_core::ue::{MapEntry, PalStruct, Property, StructValue};
use uuid::Uuid;
...
fn load_guild_dto(ctx: &mut RunContext<'_>, id: Uuid) -> Result<GuildDto, HostError> {
    ...
    // afterwards adds to -- `ps-plugin/src/host/save_write.rs:49` only
    // removes from it and `ps-core/src/domain/summaries.rs:344` is the sole
